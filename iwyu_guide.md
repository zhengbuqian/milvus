...
1. (只有第一次执行需要这个，你可以先检查是否有 iwyu_progress.md 看是不是这一步已经完成，如果已经完成，直接去第二步)先启动一个 subagent 执行这个工作： 用 find 收集和列举 internal/core 中的所有的cpp文件和头文件，包括测试文件和业务代码文件， 写到一个 .md 文件内记录进度， 每个文件一个 check box,处理好了打 x ，有问题打 ! ，还没处理留空。
2. 进入循环：
