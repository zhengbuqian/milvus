...
func EstimateEntitySize(fieldsData []*schemapb.FieldData, rowOffset int) (int, error) {
	...
	for _, fs := range fieldsData {
		...
		case schemapb.DataType_SparseFloatVector:
			...
			// counting only the size of the indices and values, ignoring other
			// bytes used in proto.
			res += int(len(vec.Contents[rowOffset].Indices.Data)) * 8
		}
	}
	return res, nil
}
			...
			case *schemapb.VectorField_SparseFloatVector:
				vectors.Vectors.Data = &schemapb.VectorField_SparseFloatVector{
					SparseFloatVector: &schemapb.SparseFloatArray{
						...
						Dim:      0,
						Contents: make([]*schemapb.SparseFloatRow, 0, topK),
					},
				}
...
func SparseFloatRowDim(row *schemapb.SparseFloatRow) int64 {
	if row == nil || row.Indices == nil || row.Indices.Data == nil || len(row.Indices.Data) == 0 {
		return 0
	}
	return int64(row.Indices.Data[len(row.Indices.Data)-1]) + 1
}
...
func appendSparseFloatArraySingleRow(dst, src *schemapb.SparseFloatArray, idx int64) int64 {
	...
	return int64(8 * len(row.Indices.Data))
}
...
func ValidateSparseFloatRows(rows ...*schemapb.SparseFloatRow) error {
	for _, row := range rows {
		if row == nil {
			return errors.New("nil SparseFloatRow")
		}
		if row.Indices == nil || row.Indices.Data == nil {
			return errors.New("nil indices in SparseFloatRow")
		}
		if row.Values == nil || row.Values.Data == nil {
			return errors.New("nil values in SparseFloatRow")
		}
		if len(row.Indices.Data) != len(row.Values.Data) {
			// include length in error message:
			return fmt.Errorf("len(indices) != len(values) in SparseFloatRow: %d != %d", len(row.Indices.Data), len(row.Values.Data))
		}
		for i := 0; i < len(row.Indices.Data); i++ {
			if row.Indices.Data[i] < 0 {
				return errors.New("negative index in SparseFloatArray")
			}
		}
	}
	return nil
