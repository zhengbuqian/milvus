...
func TestPayload_ReaderAndWriter(t *testing.T) {
	...
	testSparse_OneBatch := func(t *testing.T, rows []*schemapb.SparseFloatRow, actualDim int) {
		w, err := NewPayloadWriter(schemapb.DataType_SparseFloatVector)
		require.Nil(t, err)
		require.NotNil(t, w)
	...
	t.Run("TestSparseFloatVector_emptyRow", func(t *testing.T) {
		testSparse_OneBatch(t, []*schemapb.SparseFloatRow{
			{
	...
	t.Run("TestSparseFloatVector_largeRow", func(t *testing.T) {
		...
		testSparse_OneBatch(t, []*schemapb.SparseFloatRow{
			...
			{
				Indices: &schemapb.IntArray{Data: indices},
				Values:  &schemapb.FloatArray{Data: values},
			},
		}, dim)
	})
	...
	t.Run("TestSparseFloatVector_largeIndex", func(t *testing.T) {
		...
		testSparse_OneBatch(t, []*schemapb.SparseFloatRow{
			...
			{
				Indices: &schemapb.IntArray{Data: []int32{100, int32Max / 2, int32Max - 1}},
				Values:  &schemapb.FloatArray{Data: []float32{3.1, -3.2, 3.3}},
			},
		}, int(int32Max))
	})
