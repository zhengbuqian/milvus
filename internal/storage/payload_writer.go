...
func (w *NativePayloadWriter) AddSparseFloatVectorToPayload(data *SparseFloatVectorFieldData) error {
	...
	builder.Reserve(length)
	for i := 0; i < length; i++ {
		row := data.SparseFloatArray.Contents[i]
		elementCount := len(row.Indices.Data)
		bytesData := make([]byte, 8*elementCount)
		for j := 0; j < elementCount; j++ {
			common.Endian.PutUint32(bytesData[j*8:], uint32(row.Indices.Data[j]))
			common.Endian.PutUint32(bytesData[j*8+4:], math.Float32bits(row.Values.Data[j]))
		}
