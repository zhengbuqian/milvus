...
func RunEmbeddingFunction(task *ImportTask, data *storage.InsertData) error {
	fns := task.req.GetSchema().GetFunctions()
	for _, fn := range fns {
		runner, err := function.NewFunctionRunner(task.req.GetSchema(), fn)
		if err != nil {
			return err
		}
		...
		for i, outputFieldID := range fn.OutputFieldIds {
			outputField := typeutil.GetField(task.req.GetSchema(), outputFieldID)
