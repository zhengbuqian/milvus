	...
	for {
		...
		if float64(total) < memoryWatermark {
			log.RatedDebug(20, "skip force sync because memory level is not high enough",
				zap.String("current_total_memory_usage", logutil.ToMB(total)),
				zap.String("current_memory_watermark", logutil.ToMB(memoryWatermark)))
			return
		}
		...
		if candidate != nil {
			...
			log.Info("notify writebuffer to sync",
				zap.String("channel", candiChan), zap.String("bufferSize(MB)", logutil.ToMB(candiSize)))
		}
	}
}
