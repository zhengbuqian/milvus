...
class TestGroupChunkTranslator : public ::testing::TestWithParam<bool> {
    void SetUp() override {
        ...
        milvus_storage::PackedRecordBatchWriter writer(
            fs_, paths_, arrow_schema_, storage_config, column_groups, writer_memory);
...
TEST_P(TestGroupChunkTranslator, TestWithMmap) {
    ...
    auto fr = std::make_shared<milvus_storage::FileRowGroupReader>(
            fs_, paths_[0]);
    auto field_id_list = fr->file_metadata()->GetGroupFieldIDList().GetFieldIDList(0);
    
    row_group_meta_list.push_back(fr->file_metadata()->GetRowGroupMetadataVector());

    GroupChunkTranslator translator(
        segment_id_,
        field_metas,
        column_group_info,
        paths_,
    ...
    // estimated byte size
    for (size_t i = 0; i < translator.num_cells(); ++i) {
        auto [file_idx, row_group_idx] = translator.get_file_and_row_group_index(i);
        auto& row_group_meta = row_group_meta_list[file_idx].Get(row_group_idx);
        auto usage = translator.estimated_byte_size_of_cell(i);
        EXPECT_EQ(usage.memory_bytes, static_cast<int64_t>(row_group_meta.memory_size()));
    }
