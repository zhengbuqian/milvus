...
#include "test_cachinglayer/caching_layer_test_utils.h"
...
std::shared_ptr<Chunk> create_chunk(const FixedVector<int64_t>& data) {
    auto field_data =
...
std::shared_ptr<Chunk> create_chunk(const FixedVector<std::string>& data) {
    auto field_data =
...
protected:
    ChunkedColumnGroupTest() : int64_field_meta(FieldName(""), FieldId(0), DataType::INT64, false, std::nullopt),
                  string_field_meta(FieldName(""), FieldId(0), DataType::STRING, false, std::nullopt) {}
...
TEST_F(ChunkedColumnGroupTest, GroupChunk) {
    ...
    auto another_int64_chunk = create_chunk(FixedVector<int64_t>{11, 12, 13, 14, 15});
    EXPECT_THROW(group_chunk->AddChunk(FieldId(3), another_int64_chunk), std::exception);

    // Size
    uint64_t expected_size = int64_chunk->Size() + string_chunk->Size() + new_int64_chunk->Size();
    EXPECT_EQ(group_chunk->Size(), expected_size);

    // Cell byte size
    uint64_t expected_cell_size = int64_chunk->CellByteSize() + string_chunk->CellByteSize() + new_int64_chunk->CellByteSize();
    EXPECT_EQ(group_chunk->CellByteSize(), expected_cell_size);

    // Test empty group chunk
    auto empty_group_chunk = std::make_shared<GroupChunk>();
