...
class TestGroupChunkTranslator : public Translator<milvus::GroupChunk> {
 public:
    TestGroupChunkTranslator(std::vector<int64_t> num_rows_per_chunk,
        ...
        meta_.num_rows_until_chunk_.reserve(num_cells_ + 1);
        for (int i = 0; i < num_cells_; ++i) {
            for (auto& [field_id, chunk] : chunks_[i]->GetChunks()) {
                if (i == 0) {
                    meta_.num_rows_until_chunk_[field_id].push_back(0);
                }
                meta_.num_rows_until_chunk_[field_id].push_back(
                    meta_.num_rows_until_chunk_[field_id][i] +
                    num_rows_per_chunk[i]);
            }
        }
        key_ = key;
    }
