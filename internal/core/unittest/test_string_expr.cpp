...
// Benchmark: Regex matching through Milvus segment + expr framework.
// Reads data from /tmp/regex_bench_data.txt (generated by Python with
// the same seed/logic as the E2E and ClickHouse benchmarks).
// Generate it with: python3 -c "... (see regex_benchmark.py)"
TEST(StringExpr, RegexMatchSegmentBenchmark) {
    // Read benchmark data from file
    std::vector<std::string> strings;
    {
        std::ifstream f("/tmp/regex_bench_data.txt");
        if (!f.is_open()) {
            std::cerr << "SKIP: /tmp/regex_bench_data.txt not found. "
                      << "Generate it first." << std::endl;
            GTEST_SKIP();
        }
        std::string line;
    ...
    const int TOTAL = strings.size();
    std::cerr << "\n[BENCH-SEG] Read " << TOTAL << " rows from file"
              << std::endl;
    ...
    auto gen_regex_plan =
        ...
        auto expr = test::GenExpr().release();
        expr->set_allocated_unary_range_expr(unary_range_expr);
        proto::plan::VectorType vt = proto::plan::VectorType::FloatVector;
        auto anns = GenAnns(expr, vt, fvec_meta.get_id().get(), "$0");
        auto plan_node = std::make_unique<proto::plan::PlanNode>();
        plan_node->set_allocated_vector_anns(anns);
        return plan_node;
    };

    // Insert into growing segment in batches, overriding str field
    auto seg = CreateGrowingSegment(schema, empty_index_meta);
    const int BATCH = 1000;
    for (int batch_start = 0; batch_start < TOTAL; batch_start += BATCH) {
        int batch_end = std::min(batch_start + BATCH, TOTAL);
        int batch_size = batch_end - batch_start;

        auto raw_data = DataGen(schema, batch_size, batch_start / BATCH);
        // Override str field in raw_ proto directly
        for (int fi = 0; fi < raw_data.raw_->fields_data_size(); ++fi) {
            auto* fd = raw_data.raw_->mutable_fields_data(fi);
            if (fd->field_id() == str_meta.get_id().get()) {
                auto* sd = fd->mutable_scalars()->mutable_string_data();
                sd->clear_data();
                for (int i = batch_start; i < batch_end; ++i) {
                    sd->add_data(strings[i]);
                }
                break;
            }
        }

        seg->PreInsert(batch_size);
        seg->Insert(batch_start, batch_size,
                    raw_data.row_ids_.data(),
                    raw_data.timestamps_.data(),
                    raw_data.raw_);
    }
    ...
    // Expected counts from E2E/ClickHouse benchmarks
    struct TestCase {
        std::string name;
        std::string pattern;
        int expected_count;
    ...
    std::vector<TestCase> cases = {
        {"P1: literal 'error'",    "error",                    1997},
        {"P2: ^user_[0-9]+",       "^user_[0-9]+",            20000},
        {"P3: email-like",         "user_[0-9]+@gmail\\.com",  3236},
        {"P4: alternation",        "error|warning|critical",   26011},
        {"P5: case-insensitive",   "(?i)error",               11997},
        {"P6: multi-wildcard",     ".*error.*timeout.*",       1997},
        {"P7: pure pattern",       "[a-z]{3,5}_[0-9]+",       30000},
    };

    const int WARMUP = 3;
    const int RUNS = 10;

    std::cerr << "[BENCH-SEG] " << TOTAL
              << " rows, growing segment, no index (Volnitsky+RE2)"
              << std::endl;

    for (const auto& tc : cases) {
        auto plan_proto = gen_regex_plan(tc.pattern);
        auto plan = ProtoParser(schema).CreatePlan(*plan_proto);
        auto expr_node =
            plan->plan_node_->plannodes_->sources()[0]->sources()[0];

        for (int w = 0; w < WARMUP; ++w) {
            ExecuteQueryExpr(expr_node, seg_promote, TOTAL, MAX_TIMESTAMP);
        }
        ...
        for (int r = 0; r < RUNS; ++r) {
            ...
            auto result = ExecuteQueryExpr(
                expr_node, seg_promote, TOTAL, MAX_TIMESTAMP);
        ...
        bool count_ok = (cnt == tc.expected_count);
        std::cerr << "[BENCH-SEG] " << tc.name
                  << " | p50=" << p50 << "ms"
                  << " cnt=" << cnt
                  << (count_ok ? " OK" : " COUNT_MISMATCH")
                  << std::endl;
        EXPECT_EQ(cnt, tc.expected_count) << tc.name;
    }
}

// Raw buffer benchmark: same layout as ClickHouse ColumnString
// (contiguous chars + offsets), bypassing Milvus segment/expr framework.
// Isolates algorithm performance from data layout/framework overhead.
TEST(StringExpr, RegexMatchRawBufferBenchmark) {
    // Read data from file
    std::vector<std::string> strings;
    {
        std::ifstream f("/tmp/regex_bench_data.txt");
        if (!f.is_open()) {
            std::cerr << "SKIP: /tmp/regex_bench_data.txt not found" << std::endl;
            GTEST_SKIP();
        }
        std::string line;
        while (std::getline(f, line)) strings.push_back(std::move(line));
    }

    // Build contiguous buffer (like ClickHouse ColumnString)
    std::vector<char> chars;
    std::vector<size_t> offsets;
    {
        size_t total_bytes = 0;
        for (const auto& s : strings) total_bytes += s.size();
        chars.reserve(total_bytes);
    ...
    const size_t num_rows = strings.size();
    std::cerr << "\n[BENCH-RAW] " << num_rows << " rows, "
              << chars.size() << " bytes in contiguous buffer" << std::endl;
    ...
    for (const auto& tc : cases) {
        // Pre-build matcher and searcher (like segment-level cache)
        PartialRegexMatcher matcher(tc.pattern);
        auto lits = milvus::index::extract_literals_from_regex(tc.pattern);
        std::string best_lit;
        for (const auto& l : lits)
            if (l.size() > best_lit.size()) best_lit = l;

        std::unique_ptr<milvus::VolnitskySearcher> searcher;
        if (!best_lit.empty())
            searcher = std::make_unique<milvus::VolnitskySearcher>(best_lit);
        ...
        for (int w = 0; w < WARMUP; ++w) {
            ...
            for (size_t i = 0; i < num_rows; ++i) {
                size_t len = offsets[i] - prev;
                std::string_view sv(&chars[prev], len);
                if (searcher) {
                    if (searcher->contains(sv) && matcher(sv)) ++cnt;
                } else {
                    if (matcher(sv)) ++cnt;
                }
                prev = offsets[i];
        ...
        bool count_ok = (final_cnt == tc.expected_count);
        std::cerr << "[BENCH-RAW] " << tc.name
                  << " | p50=" << p50 << "ms"
                  << " cnt=" << final_cnt
                  << (count_ok ? " OK" : " MISMATCH")
                  << " lit='" << best_lit << "'"
                  << std::endl;
        EXPECT_EQ(final_cnt, tc.expected_count) << tc.name;
    }
}
