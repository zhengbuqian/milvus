...
PrepareInsertBinlog(int64_t collection_id,
    ...
    auto field_excluded = [&](int64_t field_id) {
        return std::find(excluded_field_ids.begin(), excluded_field_ids.end(), field_id) != excluded_field_ids.end();
    };
...
PrepareSingleFieldInsertBinlog(int64_t collection_id,
    ...
    for (auto i = 0; i < field_datas.size(); ++i) {
        ...
        auto file = "./data/test" + std::to_string(field_id) + "/" + std::to_string(i);
...
CreateSealedWithFieldDataLoaded(milvus::SchemaPtr schema,
                                ...
                                bool with_mmap = false,
                                std::vector<int64_t> excluded_field_ids = {}) {
    auto segment = milvus::segcore::CreateSealedSegment(schema, milvus::empty_index_meta);
    LoadGeneratedDataIntoSegment(dataset, segment.get(), with_mmap, excluded_field_ids);
    return segment;
}
