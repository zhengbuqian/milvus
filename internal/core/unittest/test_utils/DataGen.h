    ...
    get_col(FieldId field_id) const {
        ...
        for (auto i = 0; i < raw_->fields_data_size(); i++) {
            ...
            if constexpr (std::is_same_v<T,
                ...
                auto rows = SparseProtoToRows(sparse_float_array.contents());
...
GenerateRandomSparseFloatVector(size_t rows, size_t cols, float density, int seed = 42) {
    ...
    auto col_distrib =
        std::uniform_int_distribution<int32_t>(0, cols - 1);
...
inline auto
CreateSparseFloatPlaceholderGroup(int64_t num_queries,
                                  int64_t seed = 42) {
    namespace ser = milvus::proto::common;
    ser::PlaceholderGroup raw_group;
    auto value = raw_group.add_placeholders();
    ...
    milvus::proto::schema::SparseFloatArray sparse_float_array;
    SparseRowsToProto(sparse_vecs.get(), num_queries, &sparse_float_array);
    value->add_values(sparse_float_array.SerializeAsString());
    return raw_group;
}
