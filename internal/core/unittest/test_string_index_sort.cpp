...
const std::string TestLocalPathStr(TestLocalPath);
...
TEST_F(StringIndexSortTest, ConstructorMmap) {
    Config config;
    config["mmap_file_path"] = TestLocalPathStr + "milvus_test";
    auto index = milvus::index::CreateStringIndexSort({});
    ASSERT_NE(index, nullptr);
}
...
TEST_F(StringIndexSortTest, MmapLoadAfterSerialize) {
    ...
    mmap_config[MMAP_FILE_PATH] =
        TestLocalPathStr + "test_string_index_sort_mmap.idx";
    ...
    prefix_mmap_config[MMAP_FILE_PATH] = TestLocalPathStr + "test_prefix_mmap.idx";
    ...
    // Clean up temp files
    std::remove((TestLocalPathStr + "test_string_index_sort_mmap.idx").c_str());
    std::remove((TestLocalPathStr + "test_prefix_mmap.idx").c_str());
}
...
TEST_F(StringIndexSortTest, LoadWithoutAssembleMmap) {
    ...
    Config mmap_config;
    mmap_config[MMAP_FILE_PATH] =
        TestLocalPathStr + "test_load_without_assemble.idx";
    ...
    // Clean up
    std::remove((TestLocalPathStr + "test_load_without_assemble.idx").c_str());
}
}  // namespace index
}  // namespace milvus
...
TEST(StringIndexSortStandaloneTest, StringIndexSortBuildAndSearch) {
    ...
    {
        milvus::Config config;
        config["mmap_file_path"] = TestLocalPathStr + "milvus_scalar_test";
        auto index = milvus::index::CreateStringIndexSort({});
        index->Build(n, test_data.data());
...
TEST(StringIndexSortPatternMatchTest, PatternMatchMmap) {
    ...
    mmap_config[milvus::index::MMAP_FILE_PATH] =
        TestLocalPathStr + "test_pattern_match_mmap.idx";
    ...
    std::remove((TestLocalPathStr + "test_pattern_match_mmap.idx").c_str());
}
