...
TEST(Sealed, LoadFieldData) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (chunk_span9.get().second.size() == 0 || chunk_span9.get().second[i]) {
...
TEST(Sealed, BF) {
    ...
    auto segment = CreateSealedWithFieldDataLoaded(schema, dataset, false, {fake_id.get()});
    ...
    auto load_info = PrepareSingleFieldInsertBinlog(kCollectionID, kPartitionID, kSegmentID, fake_id.get(), {field_data}, cm);
...
TEST(Sealed, BF_Overflow) {
    ...
    auto vec_load_info = PrepareSingleFieldInsertBinlog(
        kCollectionID, kPartitionID, kSegmentID, fake_id.get(), {field_data}, cm);
...
TEST(Sealed, DeleteCount) {
    {
        ...
        auto ca = milvus::cachinglayer::SemiInlineGet(insert_record_slot->PinCells({0}));
...
TEST(Sealed, RealCount) {
    ...
    auto status =
        segment->Delete(half, del_ids1.get(), del_tss1.data());
    ASSERT_TRUE(status.ok());
    ...
    status =
        segment->Delete(half, del_ids1.get(), del_tss2.data());
    ASSERT_TRUE(status.ok());
    ASSERT_EQ(c - half, segment->get_real_count());
