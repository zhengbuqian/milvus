...
import { getRunMeta, getRunMetrics, getRunSummary, buildAssetUrl } from '../lib/api';
import { formatTimestamp, getSelectedRuns, toggleRunSelection } from '../lib/state';
import { CasesTable, CaseRow } from '../components/CasesTable';
import { formatNumber, formatPercentage, escapeHtml } from '../lib/format';
import { FlamegraphGrid } from '../components/Flamegraphs';
          ...
          <div className="section-card">
            ...
            <div className="segmented">
              {Array.from({ length: 5 }, (_, i) => i + 1).map((n) => (
                <button key={n} className={`segmented-btn${n === cols ? ' active' : ''}`} onClick={() => setCols(n)}>{n}</button>
              ))}
            </div>
          </div>
          <FlamegraphGrid cards={flameCards as any} cols={cols} />
        </div>
      )}
    </div>
  );
