...
TEST(CApiTest, MultiDeleteSealedSegment) {
    ...
    auto cm = milvus::storage::RemoteChunkManagerSingleton::GetInstance().GetRemoteChunkManager();
    auto load_info = PrepareInsertBinlog(kCollectionID, kPartitionID, kSegmentID, dataset, cm);
...
TEST(CApiTest, SealedSegment_search_float_Predicate_Range) {
    ...
    auto load_info = PrepareInsertBinlog(kCollectionID, kPartitionID, kSegmentID, dataset, cm, "", excluded_field_ids);
...
TEST(CApiTest, SealedSegment_search_without_predicates) {
    ...
    auto cm = milvus::storage::RemoteChunkManagerSingleton::GetInstance().GetRemoteChunkManager();
    auto excluded_field_ids = GetExcludedFieldIds(dataset.schema_, {0, 1, 100, 101});
    auto load_info = PrepareInsertBinlog(kCollectionID, kPartitionID, kSegmentID, dataset, cm, "", excluded_field_ids);
    status = LoadFieldData(segment, &load_info);
    ASSERT_EQ(status.error_code, Success);
    ...
    CSearchResult search_result;
    auto res =
        CSearch(segment, plan, placeholderGroup, ROW_COUNT + ts_offset, &search_result);
    std::cout << res.error_msg << std::endl;
    ASSERT_EQ(res.error_code, Success);

    CSearchResult search_result2;
    auto res2 = CSearch(
        segment, plan, placeholderGroup, ROW_COUNT + ts_offset, &search_result2);
    ASSERT_EQ(res2.error_code, Success);
...
TEST(CApiTest, RetrieveScalarFieldFromSealedSegmentWithIndex) {
    ...
    auto cm = milvus::storage::RemoteChunkManagerSingleton::GetInstance().GetRemoteChunkManager();
    auto excluded_field_ids = GetExcludedFieldIds(raw_data.schema_, {0, 1, i64_fid.get()});
    auto load_info = PrepareInsertBinlog(kCollectionID, kPartitionID, kSegmentID, raw_data, cm, "", excluded_field_ids);
    auto status = LoadFieldData(segment, &load_info);
    ASSERT_EQ(status.error_code, Success);
