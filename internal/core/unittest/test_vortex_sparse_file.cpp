...
class SingleBufferFileSystem : public arrow::fs::FileSystem {
 public:
    SingleBufferFileSystem(std::string path,
    ...
    bool
    Equals(const FileSystem& other) const override {
        return false;
    }
...
CreateSparseBuffer(const std::shared_ptr<arrow::Buffer>& full_buffer,
                   const std::vector<uint64_t>& ranges,
                   int64_t file_size,
...
VerifySparseRead(const std::shared_ptr<SingleBufferFileSystem>& sparse_fs,
    ...
    for (int i = 0; i < chunked_array->num_chunks(); i++) {
        auto struct_arr = std::dynamic_pointer_cast<arrow::StructArray>(
    ...
    std::cout << "  Verified " << field_name << ": " << total_rows << " rows OK"
              << std::endl;
}
...
class VortexSparseFileTest : public ::testing::Test {
 protected:
    ...
    SetUp() override {
        ...
        api::SetValue(properties_,
                      PROPERTY_FS_ROOT_PATH,
                      "/tmp/milvus-vortex-sparse-test");
        ...
        std::uniform_int_distribution<int64_t> i64_dist(-1000000000LL,
                                                        1000000000LL);
    ...
    MakeBatch(int32_t offset, int32_t count) {
        ...
        return arrow::RecordBatch::Make(
            schema_, count, {i32_arr, i64_arr, f32_arr, str_arr});
    }
    ...
    std::string test_file_ =
        "/tmp/milvus-vortex-sparse-test/sparse_test.vortex";
...
TEST_F(VortexSparseFileTest, SparseFileFieldScan) {
    ...
    std::cout << "Written " << kTotalRows << " rows to " << test_file_
              << std::endl;
    ...
    ASSERT_TRUE(full_buffer_result.ok())
        << full_buffer_result.status().ToString();
    ...
    for (const auto& name : field_names) {
        ...
        std::cout << "  Total Chunk (FlatLayout) children: " << total_chunks
                  << std::endl;
        size_t pos = 2;
    ...
    for (const auto& name : field_names) {
        ...
        std::cout << "Field '" << name << "': " << count << " zones segment(s)"
                  << std::endl;
    ...
    for (const auto& name : field_names) {
        auto ranges =
            vx_file.FieldByteRanges(name, static_cast<uint64_t>(file_size));
    ...
    {
        auto ranges =
            vx_file.FieldByteRanges("int32", static_cast<uint64_t>(file_size));
        std::unique_ptr<uint8_t[]> backing;
        auto sparse_buf =
            CreateSparseBuffer(full_buffer, ranges, file_size, backing);
        auto sparse_fs =
            std::make_shared<SingleBufferFileSystem>(test_file_, sparse_buf);

        VerifySparseRead(sparse_fs,
                         schema_,
                         0,
                         test_file_,
                         properties_,
                         kTotalRows,
                         [this](const std::shared_ptr<arrow::StructArray>& s,
                                int64_t offset) {
                             auto col =
                                 std::dynamic_pointer_cast<arrow::Int32Array>(
                                     s->GetFieldByName("int32"));
    ...
    {
        auto ranges =
            vx_file.FieldByteRanges("int64", static_cast<uint64_t>(file_size));
        std::unique_ptr<uint8_t[]> backing;
        auto sparse_buf =
            CreateSparseBuffer(full_buffer, ranges, file_size, backing);
        auto sparse_fs =
    ...
    {
        auto ranges =
            vx_file.FieldByteRanges("float", static_cast<uint64_t>(file_size));
        std::unique_ptr<uint8_t[]> backing;
        auto sparse_buf =
            CreateSparseBuffer(full_buffer, ranges, file_size, backing);
        auto sparse_fs =
        ...
        VerifySparseRead(
            ...
            [this](const std::shared_ptr<arrow::StructArray>& s,
                   int64_t offset) {
                auto col = std::dynamic_pointer_cast<arrow::FloatArray>(
                    s->GetFieldByName("float"));
                ASSERT_NE(col, nullptr);
                for (int64_t j = 0; j < col->length(); j++) {
    ...
    {
        auto ranges =
            vx_file.FieldByteRanges("str", static_cast<uint64_t>(file_size));
        std::unique_ptr<uint8_t[]> backing;
        auto sparse_buf =
            CreateSparseBuffer(full_buffer, ranges, file_size, backing);
        auto sparse_fs =
