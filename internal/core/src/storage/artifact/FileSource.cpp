...
class V1RemoteSource::Impl {
 public:
    ...
    BuildDirectory() {
        ...
        for (const auto& [name, path] : by_basename) {
            ...
            if (layout == V1SourceLayout::DiskFiles) {
                auto suffix = SplitNumericSuffix(name);
                if (suffix.has_value()) {
                    grouped[suffix->first].emplace_back(suffix->second, path);
                } else {
                    // Small sidecars were uploaded through MemFileManager and
                    // raw engine objects were uploaded without slicing. Both
                    // keep their exact physical name; the family decides which
                    // exact entries are engine data and which are logical
                    // sidecars.
                    const auto inserted =
                        logical_entries
                    ...
                    exact_physical_entries.insert(name);
                }
            } else {
                const auto inserted =
                    logical_entries
    ...
    std::set<std::string> exact_physical_entries;
...
void
V1RemoteSource::ReadRawEntryToLocalFile(std::string_view name,
                                        const std::string& local_path) {
    if (name.empty() || name.find('\0') != std::string_view::npos ||
        BaseName(std::string(name)) != name) {
        ThrowInfo(DataFormatBroken,
                  "raw artifact entry is not a safe basename: {}",
                  name);
    }
    if (impl_->storage_path != ArtifactStoragePath::Index ||
        impl_->layout != V1SourceLayout::DiskFiles ||
        impl_->exact_physical_entries.count(std::string(name)) == 0) {
        ThrowInfo(Unsupported,
                  "V1/V2 artifact entry {} is not an exact physical disk "
                  "object",
                  name);
    }

    StagedLocalFile staged(local_path);
    const auto& paths = impl_->Paths(name);
    AssertInfo(paths.size() == 1,
               "exact physical artifact entry {} has {} paths",
               name,
               paths.size());
    const auto expected_remote_path =
        impl_->disk_manager->GetRemoteIndexObjectPrefix() + "/" +
        std::string(name);
    if (paths.front() != expected_remote_path) {
        ThrowInfo(Unsupported,
                  "exact physical artifact entry {} resolves to {}, outside "
                  "the index file-manager prefix",
                  name,
                  paths.front());
    }
    impl_->CheckCancelled("V1/V2 raw artifact input open");
    auto input = impl_->disk_manager->OpenInputStream(std::string(name));
    if (input == nullptr) {
        ThrowInfo(FileOpenFailed, "failed to open raw artifact entry {}", name);
    }

    auto fd = ::open(staged.Staging().c_str(), O_WRONLY | O_TRUNC | O_CLOEXEC);
    if (fd < 0) {
        ThrowInfo(FileOpenFailed,
                  "failed to open staged raw artifact file {}: {}",
                  staged.Staging(),
                  std::strerror(errno));
    }
    struct FdGuard {
    ...
    std::vector<uint8_t> buffer(std::min(kCopyBufferSize, input_size));
    size_t copied = 0;
    while (copied < input_size) {
        impl_->CheckCancelled("V1/V2 raw artifact materialization");
        const auto requested = std::min(buffer.size(), input_size - copied);
        const auto read = input->Read(buffer.data(), requested);
        if (read == 0 || read > requested) {
            ThrowInfo(FileReadFailed,
                      "raw artifact entry {} returned {} of {} requested "
                      "bytes",
                      name,
                      read,
                      requested);
        ...
        while (written < read) {
            const auto result =
                ::write(fd, buffer.data() + written, read - written);
            if (result < 0 && errno == EINTR) {
                continue;
            }
            if (result <= 0) {
                ThrowInfo(FileWriteFailed,
                          "failed to write staged raw artifact file {}: {}",
                          staged.Staging(),
                          std::strerror(errno));
            }
    ...
    if (::fsync(fd) != 0) {
        ThrowInfo(FileWriteFailed,
                  "failed to flush staged raw artifact file {}: {}",
                  staged.Staging(),
                  std::strerror(errno));
    }
    ...
    if (close_result != 0) {
        ThrowInfo(FileWriteFailed,
                  "failed to close staged raw artifact file {}: {}",
                  staged.Staging(),
                  std::strerror(close_error));
    }
    impl_->CheckCancelled("V1/V2 raw artifact file publication");
    staged.Commit();
}
...
V1RemoteSource::Context() const {
    return impl_->context;
}

const std::vector<std::string>&
V1RemoteSource::RemotePaths() const {
    return impl_->remote_paths;
}
...
void
V3PackedSource::ReadRawEntryToLocalFile(std::string_view, const std::string&) {
    ThrowInfo(Unsupported,
              "V3 packed artifacts do not expose raw physical entries");
}
...
const FileManagerContext&
V3PackedSource::Context() const {
    return impl_->context;
}

const std::vector<std::string>&
V3PackedSource::RemotePaths() const {
    return impl_->remote_paths;
}

class NamedBufferSource::Impl {
 public:
    explicit Impl(const NamedBufferSet& source) {
        for (const auto& [name, buffer] : source) {
            AssertInfo(buffer.size == 0 || buffer.data != nullptr,
                       "null data for non-empty named buffer {}",
                       name);
            AssertInfo(buffer.size <= static_cast<size_t>(
                                          std::numeric_limits<int64_t>::max()),
                       "named buffer is too large: {}",
                       name);
            entries.Append(
                name, buffer.data, static_cast<int64_t>(buffer.size));
        }
        milvus::Assemble(entries);
    }

    knowhere::BinaryPtr
    Entry(std::string_view name) const {
        auto entry = entries.GetByName(std::string(name));
        if (entry == nullptr) {
            ThrowInfo(DataFormatBroken, "artifact entry not found: {}", name);
        }
        if (entry->size < 0) {
            ThrowInfo(DataFormatBroken,
                      "artifact entry {} has a negative size",
                      name);
        }
        if (entry->size != 0 && entry->data == nullptr) {
            ThrowInfo(
                DataFormatBroken, "artifact entry {} has null data", name);
        }
        return entry;
    }
...
NamedBufferSource::NamedBufferSource(const NamedBufferSet& buffers)
    : impl_(std::make_unique<Impl>(buffers)) {
}

NamedBufferSource::~NamedBufferSource() = default;

Generation
NamedBufferSource::Gen() const {
    return Generation::V1V2;
}

std::vector<std::string>
NamedBufferSource::EntryNames() const {
    std::vector<std::string> names;
    names.reserve(impl_->entries.binary_map_.size());
    for (const auto& [name, _] : impl_->entries.binary_map_) {
        names.push_back(name);
    }
    return names;
}
...
bool
NamedBufferSource::HasEntry(std::string_view name) const {
    return impl_->entries.Contains(std::string(name));
}

int64_t
NamedBufferSource::EntrySize(std::string_view name) const {
    return impl_->Entry(name)->size;
}

std::vector<uint8_t>
NamedBufferSource::ReadEntry(std::string_view name) {
    auto entry = impl_->Entry(name);
    if (entry->size == 0) {
        return {};
    }
    return std::vector<uint8_t>(entry->data.get(),
                                entry->data.get() + entry->size);
}

void
NamedBufferSource::ReadEntryToLocalFile(std::string_view name,
                                        const std::string& local_path) {
    StagedLocalFile staged(local_path);
    auto entry = impl_->Entry(name);
    WriteBytes(
        staged.Staging(), entry->data.get(), static_cast<size_t>(entry->size));
    staged.Commit();
}

void
NamedBufferSource::ReadRawEntryToLocalFile(std::string_view,
                                           const std::string&) {
    ThrowInfo(Unsupported,
              "named buffers do not define disk-vector raw file payloads");
}

void
NamedBufferSource::ReadEntriesToLocalFile(const std::vector<std::string>& names,
                                          const std::string& local_path) {
    StagedLocalFile staged(local_path);
    std::ofstream output(staged.Staging(), std::ios::binary | std::ios::trunc);
    if (!output.good()) {
    ...
    for (const auto& name : names) {
        auto entry = impl_->Entry(name);
        AppendMemory(
            entry->data.get(), static_cast<size_t>(entry->size), output, {});
    }
    CloseOutput(output, staged.Staging());
    staged.Commit();
}
...
std::vector<std::string>
NamedBufferSource::ReadEntriesToLocalDir(const std::vector<std::string>& names,
                                         const std::string& local_dir) {
    CreateParentDirectories(
        (std::filesystem::path(local_dir) / ".artifact-target").string());
    std::vector<std::string> paths;
...
std::optional<nlohmann::json>
NamedBufferSource::GetMeta(std::string_view) const {
    return std::nullopt;
}

const FileManagerContext&
NamedBufferSource::Context() const {
    return impl_->context;
}

const std::vector<std::string>&
NamedBufferSource::RemotePaths() const {
    return impl_->remote_paths;
}
