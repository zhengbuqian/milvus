...
AppendFile(knowhere::BinarySet& binary_set,
    ...
    AssertInfo(size <=
                   static_cast<size_t>(std::numeric_limits<int64_t>::max()),
               "borrowed local artifact file {} exceeds BinarySet size domain",
               local_path);
    input.seekg(0);
    auto bytes = std::shared_ptr<uint8_t[]>(new uint8_t[size]);
    if (size != 0) {
...
NamedBufferSink::Finish() {
    ...
    try {
        ...
        AssertInfo(impl_->entries.Size() <=
                       static_cast<size_t>(
                           std::numeric_limits<int64_t>::max()),
                   "serialized BinarySet size exceeds int64");
        ...
        for (const auto& [name, binary] : impl_->entries.binary_map_) {
            AssertInfo(binary != nullptr,
                       ...
                       name);
            AssertInfo(binary->size >= 0,
                       "physical named buffer entry {} has negative size",
                       name);
            AssertInfo(static_cast<uint64_t>(binary->size) <=
                           static_cast<uint64_t>(
                               std::numeric_limits<size_t>::max()),
                       "physical named buffer entry {} exceeds size_t",
                       name);
