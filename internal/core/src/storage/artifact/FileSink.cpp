...
void
AppendFile(knowhere::BinarySet& binary_set,
           std::string_view name,
           const std::string& local_path) {
    std::ifstream input(local_path, std::ios::binary | std::ios::ate);
    if (!input.good()) {
        ThrowInfo(FileOpenFailed,
                  "failed to open borrowed local artifact file {}",
                  local_path);
    }
    auto end = input.tellg();
    if (end < 0) {
        ThrowInfo(FileReadFailed,
                  "failed to get borrowed local artifact file size {}",
                  local_path);
    }
    ...
    auto bytes = std::shared_ptr<uint8_t[]>(new uint8_t[size]);
    if (size != 0) {
        input.read(reinterpret_cast<char*>(bytes.get()), size);
        if (!input.good()) {
            ThrowInfo(FileReadFailed,
                      "failed to read borrowed local artifact file {}",
                      local_path);
        }
    }
    AssertInfo(!binary_set.Contains(std::string(name)),
...
V1MemSink::V1MemSink(const FileManagerContext& context,
                     ArtifactStoragePath storage_path)
    : impl_(std::make_unique<Impl>(context, storage_path)) {
}

V1MemSink::~V1MemSink() = default;
...
void
V1MemSink::WriteEntry(std::string_view name, const void* data, size_t size) {
    AssertOpen(impl_->state);
    try {
        AppendBuffer(impl_->entries, name, data, size);
    } catch (...) {
...
void
V1MemSink::WriteEntryFromLocalFile(std::string_view name,
                                   const std::string& local_path) {
    AssertOpen(impl_->state);
    try {
        AppendFile(impl_->entries, name, local_path);
    } catch (...) {
        impl_->state = SinkState::Failed;
        throw;
    }
...
void
V1MemSink::WriteRawEntryFromLocalFile(std::string_view, const std::string&) {
    AssertOpen(impl_->state);
    impl_->state = SinkState::Failed;
    ThrowInfo(Unsupported,
              "V1/V2 in-memory transport cannot publish a raw file entry");
}

void
V1MemSink::PutMeta(std::string_view, const nlohmann::json&) {
    AssertOpen(impl_->state);
    impl_->state = SinkState::Failed;
    ThrowInfo(Unsupported,
...
V1MemSink::Finish() {
    ...
    try {
        if (!AddMemoryFiles(
                *impl_->manager, impl_->storage_path, impl_->entries)) {
            ThrowInfo(FileWriteFailed,
                      "failed to upload V1/V2 in-memory artifact entries");
        }
        auto stats =
            StatsFromRemoteMaps(impl_->manager->GetAddedTotalMemSize(),
                                impl_->storage_path,
                                impl_->manager->GetRemoteTextLogPrefix(),
                                {},
                                impl_->manager->GetRemotePathsToFileSize());
...
NamedBufferSink::NamedBufferSink() : impl_(std::make_unique<Impl>()) {
}

NamedBufferSink::~NamedBufferSink() = default;

Generation
NamedBufferSink::Gen() const {
    return Generation::V1V2;
}

void
NamedBufferSink::WriteEntry(std::string_view name,
                            const void* data,
                            size_t size) {
    AssertOpen(impl_->state);
    try {
...
void
NamedBufferSink::WriteEntryFromLocalFile(std::string_view name,
                                         const std::string& local_path) {
    AssertOpen(impl_->state);
    try {
        AppendFile(impl_->entries, name, local_path);
...
void
NamedBufferSink::WriteRawEntryFromLocalFile(std::string_view,
                                            const std::string&) {
    AssertOpen(impl_->state);
    impl_->state = SinkState::Failed;
    ThrowInfo(Unsupported,
              "named-buffer transport cannot publish a legacy raw file entry");
}

void
NamedBufferSink::PutMeta(std::string_view, const nlohmann::json&) {
    AssertOpen(impl_->state);
    impl_->state = SinkState::Failed;
    ThrowInfo(Unsupported,
              "named buffers use V1/V2 named-entry metadata, not V3 meta");
}

ArtifactStats
NamedBufferSink::Finish() {
    AssertOpen(impl_->state);
    try {
        milvus::Disassemble(impl_->entries);
        const auto serialized_size =
            static_cast<int64_t>(impl_->entries.Size());
        for (const auto& [name, binary] : impl_->entries.binary_map_) {
            AssertInfo(binary != nullptr,
                       "null physical named buffer entry: {}",
                       name);
            impl_->buffers.emplace(
                name,
                NamedBuffer{binary->data, static_cast<size_t>(binary->size)});
        }
        impl_->entries.clear();
        auto stats = ArtifactStats(serialized_size, {});
        impl_->state = SinkState::Finished;
        return stats;
    } catch (...) {
        impl_->state = SinkState::Failed;
...
void
NamedBufferSink::ReleaseLocalStaging() {
}

const NamedBufferSet&
NamedBufferSink::Data() const {
    AssertInfo(impl_->state == SinkState::Finished,
               "named-buffer data requested before successful Finish");
    return impl_->buffers;
}

NamedBufferSet
NamedBufferSink::Take() {
    AssertInfo(impl_->state == SinkState::Finished,
               "named-buffer data requested before successful Finish");
    return std::move(impl_->buffers);
}
