...
std::unique_ptr<EntryReader>
EntryReader::Open(std::shared_ptr<milvus::InputStream> input,
                  int64_t file_size,
                  int64_t collection_id,
                  ThreadPoolPriority priority) {
    auto reader = std::unique_ptr<EntryReader>(new EntryReader());
...
void
EntryReader::ReadFooterAndDirectory() {
...
EntryReader::GetEntryNames() const {
    return entry_names_;
}

Entry
EntryReader::ReadEntry(const std::string& name) {
...
Entry
EntryReader::ReadPlainEntry(const EntryMeta& meta) {
...
Entry
EntryReader::ReadEncryptedEntry(const EntryMeta& meta) {
...
void
EntryReader::ReadEntryToFile(const std::string& name,
...
void
EntryReader::WritePlainEntryToFile(const EntryMeta& meta,
                                   const std::string& local_path) {
    const auto& pm = meta.plain;
...
void
EntryReader::WriteEncryptedEntryToFile(const EntryMeta& meta,
                                       const std::string& local_path) {
    const auto& em = meta.enc;
    int fd = ::open(local_path.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0600);
    AssertInfo(fd != -1, "Failed to create file: {}", local_path);
...
void
EntryReader::ReadEntriesToFiles(
