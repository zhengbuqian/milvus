...
const std::string kRootPath = "/tmp/index_writer_test";
...
class IndexWriterV3Test : public testing::Test {
 public:
    void
    SetUp() override {
        auto conf = milvus_storage::ArrowFileSystemConfig();
        conf.storage_type = "local";
        conf.root_path = kRootPath;
...
TEST_F(IndexWriterV3Test, SmallMemoryEntryRoundtrip) {
...

    {
        ...
        DirectStreamWriter writer(output);
    ...
    auto reader = EntryReader::Open(input, file_size);

    auto entry = reader->ReadEntry("small_entry");
    VerifyPattern(entry.data, entry_size);
}

TEST_F(IndexWriterV3Test, LargeMemoryEntryRoundtrip) {
...
TEST_F(IndexWriterV3Test, MultipleEntriesRoundtrip) {
...
TEST_F(IndexWriterV3Test, DuplicateNameThrows) {
...
TEST_F(IndexWriterV3Test, ReadEntryToFile) {
    ...
    std::string local_file = kRootPath + "/read_entry_output.bin";
...
TEST_F(IndexWriterV3Test, ReadEntriesToFiles) {
    ...
    std::string file_a = kRootPath + "/output_a.bin";
    std::string file_b = kRootPath + "/output_b.bin";
...
TEST_F(IndexWriterV3Test, GetEntryNames) {
...
TEST_F(IndexWriterV3Test, EntryNotFound) {
...
TEST_F(IndexWriterV3Test, SmallEntryCache) {
...
TEST_F(IndexWriterV3Test, GetTotalBytesWritten) {
...
TEST_F(IndexWriterV3Test, WriteAfterFinishThrows) {
...
TEST_F(IndexWriterV3Test, MixedSizeEntries) {
