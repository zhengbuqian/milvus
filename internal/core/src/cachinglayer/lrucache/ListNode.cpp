...
ListNode::pin() {
    ...
    auto read_op = [this]() -> std::pair<bool, folly::SemiFuture<NodePin>> {
        ...
        if (state_ == State::LOADED) {
            return std::make_pair(false, NodePin(this));
        }
        return std::make_pair(false, load_promise_->getSemiFuture().deferValue([this](auto&&) {
            return NodePin(this);
        }));
    };
    {
    ...
    return std::make_pair(true, load_promise_->getSemiFuture().deferValue([this](auto&&) {
        return NodePin(this);
    }));
}
...
ListNode::set_error(folly::exception_wrapper error) {
    ...
    AssertInfo(state_ != State::NOT_LOADED && state_ != State::ERROR,
                ...
                state_to_string(state_));
    // may be successfully loaded by another thread.
    if (state_ == State::LOADED) {
        return;
    }
