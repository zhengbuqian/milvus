...
//
// See core_refactor/01-scalar-index.md §7 (growing interface), §7.1 (the vector
// appender and its `IndexBase` leak), §4.3 (the growing asymmetry) and §11.2
// item 3 (`IndexBase` retires within refactor phase 1, growing exits included).
//
// -------------------------------------------------------------------------
...
// base class is the UNION of two families' interfaces: of its five pure
// virtuals, three are vector-only and two are scalar-only, and each subclass
// throws away the other half (`ScalarFieldIndexing::AppendSegmentIndexDense` /
// `Sparse` / `GetDataFromIndex` throw at `:152,161,183`;
// `VectorFieldIndexing`'s two `AppendSegmentIndex` overloads throw at
// `:294,303`). That is the same Liskov violation as `IndexBase`, just on the
// growing side, and it proves the split the design draws: THE APPENDER
// INTERFACE IS SHARED BY BOTH FAMILIES, THE APPEND SIGNATURE IS NOT.
//
// -------------------------------------------------------------------------
...
// After refactor phase 1 the read side of a growing index is
// `ReaderSnapshot()`, which is a typed `shared_ptr<const ReaderT>` and needs no
// handle type at all.
//
// -------------------------------------------------------------------------
...
class GrowingIndexSet {
 public:
    ...
    // interfaces are unified across the two families; the method sets INSIDE
    // each interface are split by family").
    //
    ...
    // ---- Insert path (Appender interface) -----------------------------------
