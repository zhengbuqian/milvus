...
// segment has, holds their cache slots, and hands out typed pinned faces.
//
...
//       cost        : two pointers
//
// So the query face is NOT a per-query proxy. It is an interface view of the
// long-lived index object. §4.3 spells the query-time sequence out:
//
...
// (`index/Index.h:167`); after W1 it is `CacheSlot<index::IndexReaderBase>` —
// `IndexReaderBase` takes over `IndexBase`'s type-erased handle role (§11.2
// item 3). The index object lives INSIDE the slot and can be evicted, which is
// exactly why taking a face is pin-then-cast and never cast-then-pin: before
// the pin the object may not exist at all.
//
// ON `dynamic_cast`: the sibling cast below IS a `dynamic_cast`, and that is
// the design (§4 note: "root->face is a single sibling cast"). §10 rule 3 /
// the W1 exit criterion "exec's `dynamic_cast` to concrete indexes drops to
// zero" is about EXEC casting to CONCRETE INDEX CLASSES; it is not a ban on
// the one root->interface cast that this pin exit performs.
...
class IndexInventory {
 public:
    ...
    struct Entry {
        ...
        // The type-erased root. Holding `CacheSlot<IndexReaderBase>` (not
    ...
    // ---- Pin exits: one per query face (§4.3 step 3) ------------------------
    // Every one of these is "pin the slot, sibling-cast the root to the face,
    // wrap in `Pinned<>`", plus the §4.1 consistency assertion
    //     Assert(SameCaps(reader->Caps(), entry.meta.caps))
    // An empty `Pinned<>` means the face is absent — never an exception (§3
    // principle 3, §10 rule 4).
    //
    ...
    // idempotent, called only after the path is committed) already has; W1
    // moves it out of exec and into here. The per-batch `dynamic_cast`s that
    // DO exist today are on the by-offsets path
    ...
    PinScalarPredicate(milvus::OpContext* op_ctx, const IndexKey& key) const {
        ...
        // on `pinned_index_[0]`).
        return PinFace<index::ScalarPredicateReader<T>>(op_ctx, key);
    }
    ...
    PinPatternMatch(milvus::OpContext* op_ctx, const IndexKey& key) const {
        ...
        return PinFace<index::PatternMatchReader>(op_ctx, key);
    }
    ...
    PinTextMatch(milvus::OpContext* op_ctx, const IndexKey& key) const {
        ...
        return PinFace<index::TextMatchReader>(op_ctx, key);
    }
    ...
    PinNgram(milvus::OpContext* op_ctx, const IndexKey& key) const {
        ...
        return PinFace<index::NgramReader>(op_ctx, key);
    }
    ...
    PinSpatial(milvus::OpContext* op_ctx, const IndexKey& key) const {
        ...
        return PinFace<index::SpatialReader>(op_ctx, key);
    }

    // Unconditional cross-family face: every scalar family really implements
    // it, so it carries no `ReaderCaps` bit (§5 "cross-family face"). Present
    // whenever the field has any scalar index at all — including RTree, whose
    // `IsNull`/`IsNotNull` are real implementations on a live path
    // (`PhyNullExpr` -> `ProcessIndexChunksForValid`, Expr.h:2457).
    Pinned<index::NullReader>
    PinNull(milvus::OpContext* op_ctx, const IndexKey& key) const {
        return PinFace<index::NullReader>(op_ctx, key);
    }
    ...
    PinValue(milvus::OpContext* op_ctx, const IndexKey& key) const {
        ...
        return PinFace<index::ScalarValueReader<T>>(op_ctx, key);
    }
    ...
    PinJson(milvus::OpContext* op_ctx, const IndexKey& key) const {
        ...
        return PinFace<index::JsonIndexReader>(op_ctx, key);
    }

 private:
    // The single implementation of "pin the root, sibling-cast to the face".
    //
    // Body (to write):
    //   auto it = entries_.find(key);
    //   if (it == entries_.end() || it->second.slot == nullptr) return {};
    //   auto accessor = SemiInlineGet(it->second.slot->PinCells(op_ctx, {0}));
    //   const index::IndexReaderBase* root = accessor->get_cell_of(0);
    //   // §4.1 consistency assertion — the caps the path decision was made on
    //   // must be the caps the object actually reports.
    //   AssertInfo(SameCaps(root->Caps(), it->second.meta.caps),
    //              "inventory caps diverged from reader caps for field {}",
    //              key.field_id.get());
    //   const auto* face = dynamic_cast<const Face*>(root);   // sibling cast
    //   if (face == nullptr) return {};
    //   return Pinned<Face>(std::move(accessor), face);
    //
    ...
    Pinned<Face>
    PinFace(milvus::OpContext* op_ctx, const IndexKey& key) const;
...
//
// 1. THE ELEMENT -> ROW MAPPING (`ArrayOffsetsSealed`). §5.8 "元素→行映射的
//    归属：列的派生物，索引不持有": it is produced by COLUMN load
...
//    off the index root (§4.2); accounting belongs to the load-side translator.
