...
IndexIdentity
IndexIdentity::Persisted(int64_t index_id) {
    return IndexIdentity(index_id);
}

IndexIdentity
IndexIdentity::Local(uint64_t registration_id) {
    return IndexIdentity(registration_id);
}

IndexIdentity::Kind
IndexIdentity::kind() const {
    return value_.index() == 0 ? Kind::Persisted : Kind::Local;
}
...
IndexKeyHash::operator()(const IndexKey& key) const {
    ...
    if (key.identity.kind() == IndexIdentity::Kind::Persisted) {
        identity_hash =
...
FieldIndexCapability::FieldIndexCapability(
    ...
    : field_id_(field_id), entries_(std::move(entries)) {
    ...
    AssertInfo(duplicate == entries_.end(),
...
FieldIndexCapability::Find(const IndexKey& key) const {
    ...
    return it != entries_.end() && it->key == key ? &*it : nullptr;
}
