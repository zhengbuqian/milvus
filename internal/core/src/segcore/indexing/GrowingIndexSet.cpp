...
GrowingIndexSet::StageAppender(const FieldMeta& field_meta,
    ...
    if (IsStringDataType(field_meta.get_data_type()) &&
        ...
        return Appender{
            .caps = index::ReaderCaps{.text_match = true},
            .owner = std::make_unique<index::TantivyGrowingTextIndex>(
                unique_id.c_str(),
                "milvus_tokenizer",
    ...
    if (field_meta.get_data_type() == DataType::GEOMETRY &&
        ...
        return Appender{
            .caps = index::ReaderCaps{.spatial = true, .exact = false},
            .owner = std::make_unique<index::RTreeGrowingSpatialIndex>(
    ...
    auto make_owner = [&]<typename EngineType>(
                          std::shared_ptr<const index::GrowingVectorSource<
                              index::GrowingVectorStorageType<EngineType>>>
                              source) -> std::unique_ptr<index::GrowingIndex> {
    ...
    std::unique_ptr<index::GrowingIndex> owner;
    switch (data_type) {
...
GrowingIndexSet::CanReleaseVectorColumn(FieldId field_id,
    ...
    const auto* reader =
        dynamic_cast<const index::VectorReader*>(&pin.Reader());
...
GrowingIndexSet::Capability(FieldId field_id) const {
    ...
    IndexCapabilityEntry entry{
        .key = IndexKey{field_id, IndexIdentity::SegmentLocal(0)},
        .family = it->second.owner->Family(),
        .value_type = it->second.owner->ValueType(),
        .caps = it->second.caps,
    };
    return FieldIndexCapability(field_id, {std::move(entry)});
}
