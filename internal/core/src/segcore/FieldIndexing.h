...
//
// 1. LISKOV VIOLATION, SAME SHAPE AS `IndexBase` (§2.2, growing row). The base
//    class below is the UNION of two families' interfaces: of its five pure
//    virtuals
...
//    `AppendSegmentIndex` overloads). The lesson §7.1 draws: the APPENDER
...
//    refactor phase 1 retirement of `IndexBase` must include these two exits
//    or the sealed side ends up clean while growing still holds a reference
//    (§11.2 item 3, §7.1 end). Their consumers are
//    `SegmentGrowingImpl.h:252,562` and `query/SearchOnGrowing.cpp:139`; after
//    refactor phase 1 the growing read path is
//    `GrowingIndexSet::ReaderSnapshot()`, a typed `shared_ptr<const ReaderT>`
