    ...
    insert_pks(milvus::DataType data_type, ChunkedColumnInterface* data) {
        ...
        switch (data_type) {
            ...
            case DataType::VARCHAR: {
                auto num_chunk = data->num_chunks();
                for (int i = 0; i < num_chunk; ++i) {
                    auto column = reinterpret_cast<ChunkedVariableColumn<std::string>*>(data);
...
struct InsertRecord<false> : public InsertRecord<true> {
 public:
    ...
    void
    append_field_meta(FieldId field_id, const FieldMeta& field_meta, int64_t size_per_chunk) {
