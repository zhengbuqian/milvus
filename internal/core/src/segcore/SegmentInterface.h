...
class SegmentInterface {
 public:
    ...
    // per-batch casts at `:698,765`). Refactor phase 1 replaces both with the
    // typed pin exits on `segcore::IndexInventory`
    // (segcore/indexing/IndexInventory.h), which do the `IndexReaderBase` ->
    // reader interface sibling cast ONCE PER EXPRESSION NODE and return a
    // `Pinned<ReaderT>`; `index::IndexReaderBase` takes over `IndexBase`'s
    // handle role (core_refactor/01-scalar-index.md §11.2 item 3, §4.3). The
    // refactor phase 1 exit criterion is "exec's `dynamic_cast` to concrete
    // indexes drops to zero", and these two signatures are what makes those
    // casts necessary.
    //
    ...
    // `segcore/json_stats/` in refactor phase 1 and lost its
    // `ScalarIndex<std::string>` base; this accessor is unchanged, and that is
    // the point of the §1 evidence table: the query side never went through a
    // virtual, it always took the CONCRETE type from here and called
    // `ExecutorForShreddingData` / `ExecuteForSharedData` on it.
    //
    // END STATE (blocked on wide-table modelling, not on refactor phase 1): the
    // typed sub-columns become first-class columnar-format objects and this
    // accessor degrades to a LAYOUT DIRECTORY ("which sub-column holds path P,
    // cast to what, or is it only in the shared BSON blob"), which stays in
    // segcore because a JSON layout is inferred per segment from stats rather
    // than coming down the schema channel.
