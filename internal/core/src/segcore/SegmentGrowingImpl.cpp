...
SegmentGrowingImpl::FilterVectorValidOffsets(milvus::OpContext* op_ctx,
    ...
    auto pin = PinGrowingIndex(field_id);
    const milvus::OffsetMapping* mapping = nullptr;
    if (!chunks.empty() || !pin) {
        if (vec_base->is_mapping_storage()) {
            mapping = &vec_base->get_offset_mapping();
        }
    } else {
        const auto* reader =
            dynamic_cast<const index::VectorReader*>(&pin.Reader());
        AssertInfo(reader != nullptr,
                   "growing vector reader for field {} lacks nullable "
                   "mapping",
                   field_id.get());
        if (reader->HasValidData()) {
            mapping = &reader->OffsetMapping();
        }
    }

    if (mapping != nullptr && mapping->IsEnabled()) {
        mapping->FilterValidLogicalOffsets(seg_offsets,
                                           count,
