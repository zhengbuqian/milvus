...
BuildInterimReader(
    ...
    index::VectorMemBuilder<T> builder = view_data.has_value()
                                             ? index::VectorMemBuilder<T>(
                                                   DataType::NONE,
                                                   index_type,
                                                   metric_type,
                                                   knowhere::Version::
...
InterimSealedIndexTranslator::get_cells(
    ...
    if (is_sparse_) {
        ...
        switch (vec_data_type_) {
            case DataType::VECTOR_FLOAT:
                reader = BuildInterimReader<float>(pinned_chunks,
                                                   logical_rows,
                                                   physical_rows,
                                                   dim_,
                                                   validity,
            ...
            case DataType::VECTOR_FLOAT16:
                reader = BuildInterimReader<float16>(pinned_chunks,
                                                     logical_rows,
                                                     physical_rows,
                                                     dim_,
                                                     validity,
            ...
            case DataType::VECTOR_BFLOAT16:
                reader = BuildInterimReader<bfloat16>(pinned_chunks,
                                                      logical_rows,
                                                      physical_rows,
                                                      dim_,
                                                      validity,
    ...
    std::vector<std::pair<cid_t, std::unique_ptr<index::IndexReaderBase>>>
        result;
