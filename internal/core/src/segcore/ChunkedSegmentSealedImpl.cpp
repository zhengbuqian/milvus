    ...
    switch (load_info.storage_version) {
        case 2:
            ...
            if (fields_.find(TimestampFieldID) != fields_.end()) {
                ...
                auto num_rows = load_info.field_infos.at(TimestampFieldID.get()).row_count;
                ...
                for (int i = 0; i < timestamp_proxy_column->num_chunks(); i++) {
                    ...
                    for (size_t j = 0; j < span.row_count(); j++) {
                        auto ts = *(int64_t*)((char*)span.data() + j * span.element_sizeof());
...
ChunkedSegmentSealedImpl::load_field_data_internal(
    ...
    for (auto& [id, info] : load_info.field_infos) {
        ...
        if (SystemProperty::Instance().IsSystem(field_id)) {
            ...
            auto data_type = field_meta.get_data_type();
            if (milvus::ChunkedColumnInterface::IsChunkedVariableColumnDataType(data_type)) {
                if (data_type == DataType::JSON) {
                    column = std::make_shared<ChunkedVariableColumn<milvus::Json>>(std::move(translator), field_meta);
                } else {
                    column = std::make_shared<ChunkedVariableColumn<std::string>>(std::move(translator), field_meta);
                }
            } else if (milvus::ChunkedColumnInterface::IsChunkedArrayColumnDataType(data_type)) {
...
// TODO(tiered storage 1): 怎么会返回num_rows_？
int64_t
        ...
        if (IsVariableDataType(data_type)) {
            if (data_type == milvus::DataType::STRING ||
                ...
                if (!is_proxy_column) {
                    ...
                    AssertInfo(var_column != nullptr, "column is not of variable type");
                    LoadStringSkipIndex(
                        field_id,
                        0,
                        *var_column);
                } else {
                    auto var_column = std::dynamic_pointer_cast<ProxyChunkColumn>(column);
                    AssertInfo(var_column != nullptr, "column is not of variable type");
                    LoadStringSkipIndex(
                        field_id,
                        0,
            ...
            for (int i = 0; i < num_chunk; ++i) {
                if (!is_proxy_column) {
                    ...
                    AssertInfo(proxy_column != nullptr, "column is not of proxy type");
...
void
ChunkedSegmentSealedImpl::init_timestamp_index(const std::vector<Timestamp>& timestamps, size_t num_rows) {
