...
class CompletedIndexTranslator final
    : public cachinglayer::Translator<index::IndexReaderBase> {
 public:
    CompletedIndexTranslator(index::IndexReaderBasePtr reader,
                             std::string key,
                             int64_t segment_id,
                             bool file_backed,
    ...
    std::vector<std::pair<cachinglayer::cid_t,
                          std::unique_ptr<index::IndexReaderBase>>>
 ...
 private:
    index::IndexReaderBasePtr reader_;
    std::string key_;
    int64_t segment_id_;
...
ChunkedSegmentSealedImpl::vector_search(SearchInfo& search_info,
    ...
    if (get_bit(snapshot->binlog_index_bitset, field_id)) {
        ...
        auto* reader =
            dynamic_cast<const index::VectorReader*>(vector_pin.get());
...
ChunkedSegmentSealedImpl::get_vector(milvus::OpContext* op_ctx,
    ...
    auto* reader = dynamic_cast<const index::VectorReader*>(vector_pin.get());
    AssertInfo(reader != nullptr,
...
ChunkedSegmentSealedImpl::CreateTextIndexWithSchema(
    ...
    auto produce_text_batches = [&](const index::TextIndexBuildSource::
        ...
        if (column) {
            ...
            auto* value_reader =
                dynamic_cast<const index::ScalarValueReader<std::string_view>*>(
                    root_pin.get());
    ...
    auto artifact = std::move(index::TextIndexBuilder(std::move(params)))
                        .Build(source);
    auto reader = index::ConsumeIndexArtifact(std::move(artifact));
    ...
    auto slot = cachinglayer::Manager::GetInstance()
                    .CreateCacheSlot<index::IndexReaderBase>(
                        std::move(translator), op_ctx);
