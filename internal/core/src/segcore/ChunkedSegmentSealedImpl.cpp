...
ChunkedSegmentSealedImpl::LoadFieldData(const LoadFieldDataInfo& load_info) {
    ...
    for (auto& [id, info] : load_info.field_infos) {
        ...
        if (SystemProperty::Instance().IsSystem(field_id)) {
            ...
            std::unique_ptr<milvus::cachinglayer::Translator<
                milvus::segcore::InsertRecord<true>>>
                translator = std::make_unique<
                    storagev1translator::InsertRecordTranslator>(
                    this->get_segment_id(),
                    ...
                    insert_files,
                    this);
            insert_record_slot_ = milvus::cachinglayer::Manager::GetInstance()
...
ChunkedSegmentSealedImpl::num_chunk_data(FieldId field_id) const {
    return get_bit(field_data_ready_bitset_, field_id)
               ? fields_.find(field_id) != fields_.end()
                     ? pin_column(field_id)->get_cell_of(0)->num_chunks()
                     : 1
               : 0;
}
...
ChunkedSegmentSealedImpl::chunk_size(FieldId field_id, int64_t chunk_id) const {
    return get_bit(field_data_ready_bitset_, field_id)
               ? fields_.find(field_id) != fields_.end()
                     ? pin_column(field_id)->get_cell_of(0)->chunk_row_nums(
                           chunk_id)
                     : num_rows_.value()
               : 0;
}
...
ChunkedSegmentSealedImpl::get_chunk_by_offset(FieldId field_id,
    ...
    return pin_column(field_id)->get_cell_of(0)->GetChunkIDByOffset(offset);
}
...
ChunkedSegmentSealedImpl::vector_search(SearchInfo& search_info,
    ...
    if (get_bit(binlog_index_bitset_, field_id)) {
        ...
        auto cell_accessor = pin_column(field_id);
        auto vec_data = cell_accessor->get_cell_of(0);
...
ChunkedSegmentSealedImpl::search_sorted_pk(const PkType& pk,
    ...
    auto cell_accessor = pin_column(pk_field_id);
    auto pk_column = cell_accessor->get_cell_of(0);
    ...
    switch (schema_->get_fields().at(pk_field_id).get_data_type()) {
        ...
        case DataType::VARCHAR: {
            ...
            // get varchar pks
            auto var_column =
                dynamic_cast<ChunkedVariableColumn<std::string>*>(pk_column);
...
ChunkedSegmentSealedImpl::find_first(int64_t limit,
    ...
    if (!is_sorted_by_pk_) {
        auto ir_accessor = pin_insert_record();
        return ir_accessor->get_cell_of(0)->pk2offset_->find_first(limit, bitset);
    }
...
ChunkedSegmentSealedImpl::CreateTextIndex(FieldId field_id) {
...

    {
        // build
        auto cell_accessor = pin_column(field_id);
        if (cell_accessor != nullptr) {
            auto column = dynamic_cast<ChunkedVariableColumn<std::string>*>(
                cell_accessor->get_cell_of(0));
            AssertInfo(column != nullptr,
                       "failed to create text index, field is not of text "
                       "type: {}",
                       field_id.get());
...
ChunkedSegmentSealedImpl::get_raw_data(FieldId field_id,
    ...
    auto cell_accessor = pin_column(field_id);
    auto column = cell_accessor->get_cell_of(0);
    ...
    switch (field_meta.get_data_type()) {
        ...
        case DataType::BOOL: {
            bulk_subscript_impl<bool>(column,
                                      seg_offsets,
                                      count,
                                      ret->mutable_scalars()
                                          ->mutable_bool_data()
        ...
        case DataType::INT8: {
            bulk_subscript_impl<int8_t>(column,
                                        seg_offsets,
                                        count,
                                        ret->mutable_scalars()
                                            ->mutable_int_data()
        ...
        case DataType::INT16: {
            bulk_subscript_impl<int16_t>(column,
                                         seg_offsets,
                                         count,
                                         ret->mutable_scalars()
                                             ->mutable_int_data()
        ...
        case DataType::INT32: {
            bulk_subscript_impl<int32_t>(column,
                                         seg_offsets,
                                         count,
                                         ret->mutable_scalars()
                                             ->mutable_int_data()
        ...
        case DataType::INT64: {
            bulk_subscript_impl<int64_t>(column,
                                         seg_offsets,
                                         count,
                                         ret->mutable_scalars()
                                             ->mutable_long_data()
        ...
        case DataType::FLOAT: {
            bulk_subscript_impl<float>(column,
                                       seg_offsets,
                                       count,
                                       ret->mutable_scalars()
                                           ->mutable_float_data()
        ...
        case DataType::DOUBLE: {
            bulk_subscript_impl<double>(column,
                                        seg_offsets,
                                        count,
                                        ret->mutable_scalars()
                                            ->mutable_double_data()
...
ChunkedSegmentSealedImpl::bulk_subscript(
    ...
    auto field = reinterpret_cast<const ChunkedVariableColumn<Json>*>(column);
    for (int64_t i = 0; i < count; ++i) {
        auto offset = seg_offsets[i];
    ...
    if (!ir_accessor->get_cell_of(0)->empty_pks()) {
        auto end = std::remove_if(
            ...
            [&](const std::tuple<Timestamp, PkType>& record) {
                return !ir_accessor->get_cell_of(0)->contain(std::get<1>(record));
            });
...
ChunkedSegmentSealedImpl::LoadSegmentMeta(
    ...
    auto ir_accessor = pin_insert_record();
    ir_accessor->get_cell_of(0)->timestamp_index_.set_length_meta(std::move(slice_lengths));
...
ChunkedSegmentSealedImpl::generate_interim_index(const FieldId field_id) {
    ...
    try {
        ...
        std::shared_ptr<milvus::cachinglayer::CellAccessor<ChunkedColumnBase>> cell_accessor{nullptr};
        {
            ...
            vec_data = cell_accessor->get_cell_of(0);
        }

        auto dim =
            is_sparse ? dynamic_cast<ChunkedSparseFloatColumn*>(vec_data)->Dim()
                      : field_meta.get_dim();
