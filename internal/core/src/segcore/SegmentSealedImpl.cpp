...
SegmentSealedImpl::get_raw_data(FieldId field_id,
    ...
    switch (field_meta.get_data_type()) {
        ...
        case DataType::VECTOR_SPARSE_FLOAT: {
            auto rows = static_cast<const knowhere::sparse::SparseRow<float>*>(static_cast<const void*>(column->Data()));
            auto dst = ret->mutable_vectors()->mutable_sparse_float_vector();
            std::vector<knowhere::sparse::SparseRow<float>> tmp(count);
            for (int64_t i = 0; i < count; ++i) {
                auto offset = seg_offsets[i];
            ...
            SparseRowsToProto(tmp.data(), count, dst);
