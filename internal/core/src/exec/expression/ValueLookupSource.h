 ...
 public:
    ...
    PinnedValueLookup(const segcore::SegmentInternalInterface* segment,
        ...
        AssertInfo(
            ...
            field_id.get());
        const index::IndexReaderBase* reader = nullptr;
    ...
    template <typename T>
    const index::ScalarValueReader<T>*
    Reader() const {
        return dynamic_cast<const index::ScalarValueReader<T>*>(reader_);
    }
 ...
 private:
    ...
    ValidateReader(FieldId field_id,
        ...
        const auto validate = [&]<typename T>() {
            auto* selected =
                dynamic_cast<const index::ScalarValueReader<T>*>(reader);
            AssertInfo(
                selected != nullptr,
