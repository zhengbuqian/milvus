...
// with `FieldIndexCapability` is NOT a W1 prerequisite.
//
...
enum class ExprExecPath {
    ...
    ScalarIndex,  // a pinned index face
    PkIndex,      // segment_->pk_range / search_ids
    TextIndex,    // the text-match face
    // JsonStats: the shredded JSON layout. NOTE (§1): this is NOT an index
    // path — after W1 `JsonKeyStats` is a column layout under
    ...
    // exec's JSON call shape this wave. When the sub-columns are promoted to
    // first-class columnar-format objects this value folds into `RawData`.
    JsonStats,
};

// Which query face this expression needs. One-to-one with the faces in
// `index/contracts/` (§4 contract tree).
enum class RequiredFace {
...
struct ExprIndexRequirement {
    FieldId field_id;
    RequiredFace face{RequiredFace::Predicate};
...
// is asked AFTER the pin, on the pinned face. It is still self-description and
// still not a throw (§3 principle 3). `caps.ngram_candidates` decides whether
// to pin at all; `CanHandle` decides whether this particular call uses the pin.
//
...
// on the pinned face too, not in `ReaderCaps`.
