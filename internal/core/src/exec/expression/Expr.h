...
// the by-offsets path (`:698,765`). After refactor phase 1 the caller pins THE
// READER INTERFACE IT NEEDS, once per expression node, and gets a
// `segcore::Pinned<ReaderT>` back:
...
class SegmentExpr : public Expr {
 public:
    ...
    ProcessIndexChunksImpl(FUNC func,
        ...
        if (need_element_slicing) {
            ...
            // exec/expression/CandidateRefine.h (refactor phase 4; §5.8 /
            // §12.5).
            //
 ...
 public:
    ...
    // question asked on the PINNED interface — the same two-level shape as
    ...
    // then pin the ONE interface the decision chose. Two things go away in the
    // move: the "pin, then check whether the pin actually produced something"
    // dance below, and every subclass's "refine by asking the pinned concrete
    // index" tail (`CanUseIndexForOp` / `PinnedJsonIndexIsFlat` / the
    // `Support*` probes) — all of which are caps reads that belong BEFORE the
    // pin. See core_refactor/01-scalar-index.md §4.1, §4.3, §9 and §10 rule 3b.
    virtual void
    DetermineExecPath() {
