...
class SegmentExpr : public Expr {
                        ...
                        if (ExprResCacheManager::IsEnabled()) {
                            auto validity = ExprCacheHelper::GetOrComputeBitmap(
                                segment_, signature, active_count_, [&]() {
                                    return json_reader_->Exists(
                                        json_pointer,
                                        json_value_type.value());
                                });
                            valid_res = std::move(*validity);
                        } else {
                            valid_res = json_reader_->Exists(
                                json_pointer,
                                json_value_type.value());
                        }
 ...
 protected:
    ...
    template <typename T>
    bool
    ExposesPredicateReader(const index::IndexReaderBase* reader) const {
        return dynamic_cast<const index::ScalarPredicateReader<T>*>(reader) !=
               nullptr;
    }

    bool
    ExposesPredicateReader(DataType type,
                           const index::IndexReaderBase* reader) const {
        switch (type) {
            case DataType::BOOL:
    ...
    template <typename T>
    bool
    ExposesValueReader(const index::IndexReaderBase* reader) const {
        return dynamic_cast<const index::ScalarValueReader<T>*>(reader) !=
               nullptr;
    }
    ...
    template <typename T>
    const index::ScalarPredicateReader<index_value_t<T>>*
    ...
    template <typename T>
    const index::ScalarValueReader<index_value_t<T>>*
    ValueReader() const {
        return dynamic_cast<const index::ScalarValueReader<index_value_t<T>>*>(
            selected_reader_);
    }
    ...
    SelectAndPinIndex(const ExprIndexRequirement& req) {
        ...
        if (entry->caps.json_paths) {
            json_reader_ =
                dynamic_cast<const index::JsonIndexReader*>(selected_reader_);
        ...
        if (req.reader != RequiredReader::JsonPath) {
            ...
            if (reader_caps.pattern_match) {
                pattern_reader_ = dynamic_cast<const index::PatternMatchReader*>(
                    selected_reader_);
            }
            if (reader_caps.text_match) {
                text_reader_ = dynamic_cast<const index::TextMatchReader*>(
                    selected_reader_);
            }
            if (reader_caps.ngram_candidates) {
                ngram_reader_ =
                    dynamic_cast<const index::NgramReader*>(selected_reader_);
            }
            if (reader_caps.spatial) {
                spatial_reader_ =
                    dynamic_cast<const index::SpatialReader*>(selected_reader_);
            }
            null_reader_ =
                dynamic_cast<const index::NullReader*>(selected_reader_);
        }
 ...
 public:
    ...
    // NgramReader::CanHandle and PatternMatchReader::ShouldUseForOp express the
 ...
 protected:
    ...
    const index::IndexReaderBase* selected_reader_{nullptr};
    std::optional<segcore::IndexCapabilityEntry> selected_index_entry_;
    const index::JsonIndexReader* json_reader_{nullptr};
    const index::NullReader* null_reader_{nullptr};
    const index::PatternMatchReader* pattern_reader_{nullptr};
    const index::TextMatchReader* text_reader_{nullptr};
    const index::NgramReader* ngram_reader_{nullptr};
    const index::SpatialReader* spatial_reader_{nullptr};
