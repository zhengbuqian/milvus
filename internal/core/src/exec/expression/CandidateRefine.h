...
// GISFunctionFilterExpr.cpp:480-560). W1 changes only the contract underneath
// it: `SpatialReader::Candidates(SpatialOp, const Geometry&)` returning a
// `TargetBitmap` instead of `QueryCandidates(proto GISOp, Geometry,
// std::vector<int64_t>&)`. The coarse/refine skeleton itself does not move.
//
...
// for every face: selectivity is a runtime property of the query, not a static
// property of a family, and of the two shapes the bitmap is the one that does
// not explode when it degenerates.)
//
// -------------------------------------------------------------------------
...
//            production `index -> exec` includes W1 must zero.)
//
...
// -------------------------------------------------------------------------
// ELEMENT -> ROW PROJECTION. NOT IMPLEMENTED THIS WAVE — READ BEFORE ADDING A
// FOURTH COPY.
//
...
// IS W4 WORK AND IS DELIBERATELY NOT DONE HERE (§5.8 "what this wave converges"
// and §12.5). W1 only has to guarantee the index side delivers clean
// element-level results. §12.5 also records the condition that would pull it
// forward: if W1 gives MORE families element-level output, the three scattered
// folds start meeting combinations they have never covered, and the operator
// must land at the end of W1 instead of in W4. The check is the §8 family list:
// if no family newly gains element-level output, W4 is the right home.
//
