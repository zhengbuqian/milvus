...
class SealedDataGetter : public DataGetter<OutputType> {
 private:
    ...
    const index::ScalarValueReader<IndexValueType>* value_reader_{nullptr};
 ...
 private:
    ...
    InitValueReader(const std::optional<std::string>& json_path,
        ...
        const index::IndexReaderBase* reader = index_pin_.get();
        if constexpr (is_json) {
            if (entry->caps.json_paths) {
                auto* json_reader =
                    dynamic_cast<const index::JsonIndexReader*>(reader);
        ...
        value_reader_ =
            dynamic_cast<const index::ScalarValueReader<IndexValueType>*>(reader);
        AssertInfo(value_reader_ != nullptr,
