 ...
 public:
    ...
    Size() const override {
        ...
        for (size_t i = 0; i < length(); ++i) {
            data_size += SparseRowSerializedSize(field_data_[i]);
        }
        return data_size;
    }
    ...
    Size(ssize_t offset) const override {
        ...
        AssertInfo(offset < length(),
                   "subscript position don't has valid value");
        return SparseRowSerializedSize(field_data_[offset]);
    }
    ...
    // each binary in array is a serialized knowhere::sparse::SparseRow<float>
    void
    FillFieldData(const std::shared_ptr<arrow::BinaryArray>& array) override {
        auto n = array->length();
        if (n == 0) {
        ...
        for (int64_t i = 0; i < array->length(); ++i) {
            auto view = array->GetString(i);
            auto& row = field_data_[length_ + i];
            row = DeserializeSparseRow(view.data(), view.size());
            vec_dim_ = std::max(vec_dim_, row.dim());
        }
        length_ += n;
    }
