...
class GroupChunk {
 public:
    GroupChunk() = default;
    explicit GroupChunk(const std::unordered_map<FieldId, std::shared_ptr<Chunk>>& chunks)
        : chunks_(chunks) {}

    virtual ~GroupChunk() = default;

    // Get the chunk for a specific field
    std::shared_ptr<Chunk> GetChunk(FieldId field_id) const {
    ...
    // Get all chunks
    const std::unordered_map<FieldId, std::shared_ptr<Chunk>>& GetChunks() const {
        return chunks_;
    }
    ...
    bool HasChunk(FieldId field_id) const {
