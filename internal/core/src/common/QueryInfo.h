...
// chain is `index/Index.h`, the shared base class, which has three direct
// references; breaking that one belongs to the index-side work.
//
...
// is the narrowing this design uses everywhere else: THE INTERFACE DECLARES ITS
// OWN PARAMETER TYPE. That type is `index::VectorSearchParams` in
// `index/contracts/VectorReaders.h` — the vector family's own header, where
