...
void
ValidateKnowhereBytes(size_t count, size_t element_size, const char* label) {
    if (count != 0 &&
...
ValidateRangeInput(const DatasetPtr& data_set, int64_t nq, int64_t topk) {
    ...
    ValidateKnowhereBytes(hit_count, sizeof(int64_t), "range-search id");
    ValidateKnowhereBytes(hit_count, sizeof(float), "range-search distance");
