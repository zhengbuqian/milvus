...
class ChunkedColumnGroup {
 public:
    explicit ChunkedColumnGroup(std::unique_ptr<Translator<GroupChunk>> translator)
    ...
    int64_t GetNumRowsUntilChunk(int64_t chunk_id) const {
        int64_t total_rows = 0;
    ...
    const std::vector<int64_t>& GetNumRowsUntilChunk() const {
...
class ProxyChunkColumn : public ChunkedColumnInterface {
 public:
    explicit ProxyChunkColumn(std::shared_ptr<ChunkedColumnGroup> group,
        ...
        : group_(group), field_id_(field_id), field_meta_(field_meta), data_type_(field_meta.get_data_type()) {
    }
    ...
    Span(int64_t chunk_id) const override {
        if (!IsChunkedColumnDataType(data_type_)) {
            PanicInfo(ErrorCode::Unsupported, "Span only supported for ChunkedColumn");
        }
        auto chunk_wrapper = group_->GetGroupChunk(chunk_id);
        auto chunk = chunk_wrapper.get()->GetChunk(field_id_);
        return PinWrapper<SpanBase>(chunk_wrapper, static_cast<FixedWidthChunk*>(chunk.get())->Span());
    }

    PinWrapper<std::pair<std::vector<std::string_view>, FixedVector<bool>>>
    StringViews(int64_t chunk_id,
                std::optional<std::pair<int64_t, int64_t>> offset_len = std::nullopt) const override {
        if (!IsChunkedVariableColumnDataType(data_type_)) {
            PanicInfo(ErrorCode::Unsupported, "StringViews only supported for ChunkedVariableColumn");
        }
        auto chunk_wrapper = group_->GetGroupChunk(chunk_id);
        auto chunk = chunk_wrapper.get()->GetChunk(field_id_);
        return PinWrapper<std::pair<std::vector<std::string_view>, FixedVector<bool>>>(
            chunk_wrapper, static_cast<StringChunk*>(chunk.get())->StringViews(offset_len));
    }
    ...
    ArrayViews(int64_t chunk_id,
        ...
        if (!IsChunkedArrayColumnDataType(data_type_)) {
            PanicInfo(ErrorCode::Unsupported, "ArrayViews only supported for ChunkedArrayColumn");
        }
        auto chunk_wrapper = group_->GetGroupChunk(chunk_id);
        auto chunk = chunk_wrapper.get()->GetChunk(field_id_);
        return PinWrapper<std::pair<std::vector<ArrayView>, FixedVector<bool>>>(
            chunk_wrapper, static_cast<ArrayChunk*>(chunk.get())->Views(offset_len));
    }
    ...
    ViewsByOffsets(int64_t chunk_id,
        ...
        if (!IsChunkedVariableColumnDataType(data_type_)) {
            PanicInfo(ErrorCode::Unsupported, "ViewsByOffsets only supported for ChunkedVariableColumn");
        }
        auto chunk_wrapper = group_->GetGroupChunk(chunk_id);
        auto chunk = chunk_wrapper.get()->GetChunk(field_id_);
        return PinWrapper<std::pair<std::vector<std::string_view>, FixedVector<bool>>>(
            chunk_wrapper, static_cast<StringChunk*>(chunk.get())->ViewsByOffsets(offsets));
    }
    ...
    RawAt(size_t i) const {
        if (!IsChunkedVariableColumnDataType(data_type_)) {
            PanicInfo(ErrorCode::Unsupported, "RawAt only supported for ChunkedVariableColumn");
        }
    ...
    ScalarArray
    RawAt(const int i) const override {
        if (!IsChunkedArrayColumnDataType(data_type_)) {
            PanicInfo(ErrorCode::Unsupported, "RawAt only supported for ChunkedArrayColumn");
        }
        auto [chunk_id, offset_in_chunk] = GetChunkIDByOffset(i);
        auto chunk = group_->GetGroupChunk(chunk_id).get()->GetChunk(field_id_);
