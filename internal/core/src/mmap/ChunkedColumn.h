...
std::pair<size_t, size_t> inline GetChunkIDByOffset(
  ...
  auto iter = std::lower_bound(num_rows_until_chunk.begin(),
                               num_rows_until_chunk.end(), offset + 1);
...
class ChunkedColumnBase : public ChunkedColumnInterface {
 public:
    ...
    chunk_row_nums(int64_t chunk_id) const override {
        return GetNumRowsUntilChunk(chunk_id + 1) - GetNumRowsUntilChunk(chunk_id);
    }
    ...
    ValueAt(int64_t offset) override {
        PanicInfo(ErrorCode::Unsupported, "ValueAt only supported for ChunkedColumn");
    }
...
class ChunkedColumn : public ChunkedColumnBase {
 public:
    ...
    // TODO(tiered storage 1): 这个方法应该被删掉，使用一个批量访问的接口
    const char*
    ValueAt(int64_t offset) override {
        auto [chunk_id, offset_in_chunk] = GetChunkIDByOffset(offset);
        auto ca =
...
class ChunkedVariableColumn : public ChunkedColumnBase {
 public:
    static_assert(std::is_same_v<T, std::string> || std::is_same_v<T, Json>,
    ...
    // TODO(tiered storage 1): 这个方法应该被删掉，使用一个批量访问的接口
    // RawAt 有三种调用方式：
    // 1. bulk_subscript 的时候，传入一个 offset 数组，访问指定的行。
    // 2. 加载的时候创建 skip index 或 text index，访问所有行。（SkipIndex.h 和 CreateTextIndex）
    // 3. GetJsonData, json 相关的索引会使用，可以看看是否可以改成批量访问或者批量Pin的方式。
    // 
    // 理论上现在是有bug的，当T是Json的时候，返回的是一个view，但是没有pin wrapper。但是这个PR之前也有相同的bug。
    T
