...
#include "index/contracts/build/ArtifactBuilder.h"
...
// Owns one complete compact resident tensor and every side view later borrowed
// by one typed vector builder. FieldData batches are released after Add.
class VectorBuildMaterializer final {
 public:
    ...
    // Direct C-ABI input already owns one complete compact tensor. Retain that
    // owner and expose it without allocating a second raw column.
    void
    SetBorrowedContiguous(FieldDataPtr batch);
 ...
 private:
    ...
    struct TypedState {
        ...
        std::unique_ptr<index::ArtifactBuilder<Input>> builder;
    ...
    template <typename T>
    size_t
    BorrowedElementCount(const FieldDataBase& batch) const;

    template <typename T>
    size_t
    BorrowedBatchElementCount(const FieldDataBase& batch,
                              size_t physical_rows) const;

    template <typename T>
    void
    AppendBatch(TypedState<T>& state,
    ...
    FieldDataPtr borrowed_owner_;
    std::optional<VectorScalarInfo> scalar_info_;
    bool consumed_{false};
    // Kept last so any engine-side borrower is destroyed before input owners.
    State state_;
};
