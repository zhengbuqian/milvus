...
void
BuildIndexFromDirectVector(
    CIndex index, milvus::indexbuilder::DirectVectorInput input) {
    auto& session = RequireBuildSession(index);
...
CreateIndexForUT(enum CDataType dtype,
    ...
    try {
        ...
        auto handle = MakeBuildSessionHandle(
...
BuildFloatVecIndex(CIndex index,
    ...
    try {
        BuildIndexFromDirectVector(index,
...
BuildScalarIndex(CIndex c_index, int64_t size, const void* field_data) {
    ...
    try {
        auto& session = RequireBuildSession(c_index);
        AssertInfo(session.IsDirect(),
                   "production scalar/text session cannot consume a direct "
                   "scalar payload");
        session.BuildFromFieldData(
            milvus::indexbuilder::AdaptDirectScalarFieldData(
...
SerializeIndexToBinarySet(CIndex index, CBinarySet* c_binary_set) {
    ...
    try {
        ...
        auto binary = std::make_unique<milvus::NamedBufferSet>(
            RequireBuildSession(index).SerializePhysical());
...
LoadIndexFromBinarySet(CIndex index, CBinarySet c_binary_set) {
    ...
    try {
        ...
        auto binary_set =
            reinterpret_cast<milvus::NamedBufferSet*>(c_binary_set);
        RequireBuildSession(index).LoadExistingIndex(*binary_set);
        return milvus::SuccessCStatus();
    } catch (const std::exception& error) {
        return milvus::FailureCStatus(&error);
    }
