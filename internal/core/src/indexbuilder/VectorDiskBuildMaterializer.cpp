...
class VectorDiskBuildMaterializer::Impl {
 public:
    enum class State { Feeding, PrimaryFinished, Consumed, Failed };

    Impl(std::string staging_parent,
         DataType field_type,
         DataType value_type,
         int64_t dim,
         bool nullable,
        ...
        : owner_(index::VectorDiskLocalFiles::Create(staging_parent)),
        ...
        raw_path_ = (std::filesystem::path(owner_->Directory()) /
                     (value_type_ == DataType::VECTOR_SPARSE_U32_F32
    ...
    FinishPrimary() {
        ...
        try {
            ...
            if (nullable_) {
                valid_path_ = (std::filesystem::path(owner_->Directory()) /
                               "valid_data_input")
                                  .string();
            ...
            if (embedding_list_ && !all_null_) {
                AssertInfo(offsets_.size() == physical_valid_parents_ + 1 &&
                           ...
                           "row counts");
                offsets_path_ = (std::filesystem::path(owner_->Directory()) /
    ...
    SetScalarInfo(VectorScalarInfo scalar_info) {
        ...
        try {
            ...
            auto path = (std::filesystem::path(owner_->Directory()) /
                         "opt_fields_input")
                            .string();
            OutputFile file(path);
    ...
    TakeInputs() {
        ...
        VectorDiskBuildInputs result{
            ...
            .valid_path = std::move(valid_path_),
            .offsets_path = std::move(offsets_path_),
 ...
 private:
    ...
    // The generation owner is declared before open files so descriptors close
    // before the uniquely owned directory is removed on every failure path.
    std::shared_ptr<index::VectorDiskLocalFiles> owner_;
...
VectorDiskBuildMaterializer::VectorDiskBuildMaterializer(
    ...
    int64_t expected_rows,
    bool side_input_declared)
    : impl_(std::make_unique<Impl>(std::move(staging_parent),
                                   field_type,
                                   value_type,
                                   dim,
                                   nullable,
...
VectorDiskBuildMaterializer::VectorDiskBuildMaterializer(
    VectorDiskBuildMaterializer&&) noexcept = default;
VectorDiskBuildMaterializer&
VectorDiskBuildMaterializer::operator=(VectorDiskBuildMaterializer&&) noexcept =
    default;

void
VectorDiskBuildMaterializer::Add(const FieldDataPtr& batch) {
    AssertInfo(impl_ != nullptr, "disk vector materializer was moved from");
    impl_->Add(batch);
}
