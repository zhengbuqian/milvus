...
struct DiskBuilderValueType<std::unique_ptr<
    index::ArtifactBuilder<index::PreparedVectorBuildFiles<T>>>> {
    using type = T;
};
...
VectorDiskBuildMaterializer::VectorDiskBuildMaterializer(
    ...
    switch (value_type) {
        case DataType::VECTOR_FLOAT:
            create_builder.template operator()<float>();
            break;
        case DataType::VECTOR_BINARY:
            create_builder.template operator()<bin1>();
            break;
        case DataType::VECTOR_FLOAT16:
            create_builder.template operator()<float16>();
            break;
        case DataType::VECTOR_BFLOAT16:
            create_builder.template operator()<bfloat16>();
            break;
        case DataType::VECTOR_INT8:
            create_builder.template operator()<int8>();
            break;
        case DataType::VECTOR_SPARSE_U32_F32:
            create_builder.template operator()<sparse_u32_f32>();
            break;
        default:
