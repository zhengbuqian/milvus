...
IndexBuildService::RunToArtifact() {
    ...
    try {
        ...
        const auto materialize_primary = [&](auto& materializer,
            ...
            if (accumulate_columnar) {
                add_missing_rows(
                    materializer, req_.expected_rows - columnar_rows);
...
BuildDirectInput(DataType source_type,
    ...
    AssertInfo(batch != nullptr, "direct index build received a null batch");
    AssertInfo(batch->Length() <=
                   static_cast<size_t>(std::numeric_limits<int64_t>::max()),
               "direct index-build row count exceeds int64");
    const auto expected_rows = static_cast<int64_t>(batch->Length());
    try {
        if (adapted.family == index::families::kVectorDisk) {
            AssertInfo(source_type != DataType::VECTOR_ARRAY,
                       "direct disk VECTOR_ARRAY build is unsupported");
            const auto local_dir = index::GetValueFromConfig<std::string>(
                adapted.params, "local_dir");
            AssertInfo(local_dir.has_value() && !local_dir->empty(),
                       "direct disk vector build has no staging parent");
            const auto dim =
                index::GetValueFromConfig<int64_t>(adapted.params, DIM_KEY)
                    .value_or(0);
            VectorDiskBuildMaterializer materializer(*local_dir,
                                                     source_type,
                                                     adapted.value_type,
                                                     dim,
                                                     batch->IsNullable(),
                                                     ...
                                                     adapted.family,
                                                     adapted.params);
            AssertInfo(materializer.InputSpec().side_inputs.empty(),
                       "direct disk vector build cannot supply side inputs");
            materializer.Add(batch);
            materializer.FinishPrimary();
            return BuildProduct::FromArtifact(std::move(materializer).Build());
        }
        if (adapted.family == index::families::kVectorMem) {
            const auto dim =
                index::GetValueFromConfig<int64_t>(adapted.params, DIM_KEY)
                    .value_or(0);
            VectorBuildMaterializer materializer(source_type,
                                                 adapted.value_type,
                                                 dim,
                                                 expected_rows,
                                                 adapted.family,
                                                 adapted.params);
            AssertInfo(
                materializer.InputSpec().side_inputs.empty(),
                "direct resident vector build cannot supply side inputs");
            if (source_type == DataType::VECTOR_ARRAY) {
                materializer.Add(batch);
            } else {
                materializer.SetBorrowedContiguous(batch);
            }
            return BuildProduct::FromArtifact(std::move(materializer).Build());
        }
        auto materializer = MakeScalarBuildInputMaterializer(source_type,
                                                             adapted.value_type,
                                                             expected_rows,
                                                             adapted.family,
                                                             adapted.params);
        AssertInfo(materializer->InputSpec().side_inputs.empty(),
                   "direct scalar build cannot supply side inputs");
        materializer->Add(batch);
        return BuildProduct::FromArtifact(std::move(*materializer).Build());
    } catch (const SegcoreError& error) {
        if (error.get_error_code() == DataIsEmpty) {
...
IndexBuildService::Publish(const BuildProduct& product) const {
    ...
    auto sink = MakeSink();
    product.GetArtifact().Serialize(
        *sink, storage::ArtifactSerializationMode::FullArtifact);
