...
// Native lifecycle holder for one source-backed or caller-supplied index
// build. Source materialization and artifact publication remain in
// IndexBuildService; this class preserves either a built artifact or a loaded
// query reader. It intentionally has no protobuf or legacy creator dependency.
class BuildSession final {
 public:
    ...
    // Build from one caller-supplied complete decoded FieldData. The session
    // still performs the type-specific preparation required by the selected
    // family; scalar hybrid selection and the chosen family consume the same
    // owning FieldData.
    void
    BuildFromFieldData(const FieldDataPtr& batch);

    // Open an existing index transactionally into a query reader, without a
    // builder or Artifact. An ordinary family reads the supplied V1/V2
    // BinarySet. A production disk-vector session ignores those buffers and
    // opens normalized remote INDEX_FILES; a direct disk session is rejected.
    // The loaded Reader cannot be serialized or published by this session.
    void
    LoadExistingIndex(NamedBufferSet buffers);
    ...
    // retained build outcome. An Artifact is serialized as FullArtifact
    // through the generation-specific FileSink to the configured remote object
    // store; SkippedEmpty returns empty file stats. Sink writes may upload
