...
VectorBuildMaterializer::VectorBuildMaterializer(
    ...
    switch (value_type) {
        case DataType::VECTOR_FLOAT:
            InitializeTyped<float>(family, params);
            break;
        case DataType::VECTOR_BINARY:
            InitializeTyped<bin1>(family, params);
            break;
        case DataType::VECTOR_FLOAT16:
            InitializeTyped<float16>(family, params);
            break;
        case DataType::VECTOR_BFLOAT16:
            InitializeTyped<bfloat16>(family, params);
            break;
        case DataType::VECTOR_INT8:
            InitializeTyped<int8>(family, params);
            break;
        case DataType::VECTOR_SPARSE_U32_F32:
            AssertInfo(field_type != DataType::VECTOR_ARRAY,
                       "VECTOR_ARRAY does not support sparse elements");
            InitializeTyped<sparse_u32_f32>(family, params);
            break;
        default:
...
VectorBuildMaterializer::VectorBuildMaterializer(
    ...
    : field_type_(other.field_type_),
      ...
      borrowed_owner_(std::move(other.borrowed_owner_)),
...
VectorBuildMaterializer::operator=(VectorBuildMaterializer&& other) noexcept {
    ...
    // Release the old builder while all of its possible input owners live.
    state_.emplace<0>();
    field_type_ = other.field_type_;
    value_type_ = other.value_type_;
    dim_ = other.dim_;
...
VectorBuildMaterializer::Add(const FieldDataPtr& batch) {
    AssertOpen("add to");
    AssertInfo(borrowed_owner_ == nullptr,
               "cannot append after borrowing a complete vector tensor");
    ValidateBatch(batch);
    AssertInfo(
        layout_.LogicalRows() <= CheckedSize(expected_rows_, "expected row") &&
            batch->Length() <= CheckedSize(expected_rows_, "expected row") -
                                   layout_.LogicalRows(),
...
void
VectorBuildMaterializer::SetBorrowedContiguous(FieldDataPtr batch) {
    AssertOpen("set input on");
    AssertInfo(field_type_ != DataType::VECTOR_ARRAY,
               "VECTOR_ARRAY input is not a contiguous engine tensor");
    ValidateBatch(batch);
    std::visit(
        [this, &batch](auto& state) {
            using S = std::decay_t<decltype(state)>;
            if constexpr (!std::is_same_v<S, std::monostate>) {
                AssertInfo(
                    state.values.empty() && layout_.LogicalRows() == 0,
                    "cannot borrow a vector tensor after appending input");
                AppendBatch(state, batch, false);
            }
        },
        state_);
    borrowed_owner_ = std::move(batch);
}
...
VectorBuildMaterializer::Build() && {
    AssertOpen("build");
    consumed_ = true;
    AssertInfo(layout_.LogicalRows() ==
                   CheckedSize(expected_rows_, "expected row"),
...
template <typename T>
size_t
VectorBuildMaterializer::BorrowedElementCount(
    const FieldDataBase& batch) const {
    using ValueType = typename TypedState<T>::ValueType;
    const auto physical_rows = layout_.PhysicalRows();
    ...
    AssertInfo(
        ...
        "vector tensor size overflows size_t");
    const auto bytes = row_bytes * physical_rows;
    AssertInfo(batch.DataSize() >= 0 &&
                   static_cast<uint64_t>(batch.DataSize()) == bytes,
               "vector batch has {} bytes, expected {}",
               batch.DataSize(),
               bytes);
    return bytes / sizeof(ValueType);
}

template <typename T>
size_t
VectorBuildMaterializer::BorrowedBatchElementCount(
    const FieldDataBase& batch, size_t physical_rows) const {
    using ValueType = typename TypedState<T>::ValueType;
    const auto row_bytes = vector_bytes_per_element(value_type_, dim_);
    AssertInfo(row_bytes % sizeof(ValueType) == 0,
               "vector row bytes are not aligned to physical value type");
...
VectorBuildMaterializer::AppendBatch(TypedState<T>& state,
    ...
    if (field_type_ == DataType::VECTOR_ARRAY) {
        ...
        if (copy_values) {
            ...
            if (valid_rows != 0) {
                state.values.insert(
                    state.values.end(), begin, begin + valid_rows);
            }
        }
    } else {
        AssertInfo(batch->get_dim() == dim_,
                   ...
                   dim_);
        const auto elements =
            BorrowedBatchElementCount<T>(*batch, valid_rows);
        if (copy_values) {
            if (state.values.empty() && layout_.LogicalRows() == 0 &&
                !batch->IsNullable()) {
                const auto row_elements =
                    vector_bytes_per_element(value_type_, dim_) /
                    sizeof(ValueType);
                const auto expected =
                    CheckedSize(expected_rows_, "expected row");
                AssertInfo(
            ...
            if (elements != 0) {
                state.values.insert(
                    state.values.end(), begin, begin + elements);
            }
        }
    }
...
VectorBuildMaterializer::BuildTyped(
    ...
    using ValueType = typename TypedState<T>::ValueType;
    std::span<const ValueType> physical_values;
    if (borrowed_owner_ != nullptr) {
        const auto elements = BorrowedElementCount<T>(*borrowed_owner_);
        const auto* values =
            static_cast<const ValueType*>(borrowed_owner_->Data());
        AssertInfo(values != nullptr || elements == 0,
                   "borrowed vector tensor has null physical data");
        physical_values = {values, elements};
    } else {
        physical_values = state.values;
    }
