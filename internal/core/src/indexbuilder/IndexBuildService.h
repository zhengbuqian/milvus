...
class IndexBuildService {
 public:
    ...
    // Publish the explicit build outcome. An Artifact is serialized as
    // FullArtifact through the output-generation-specific FileSink to the
    // configured remote object store; SkippedEmpty returns empty file stats.
    // Writes may upload incrementally; Finish finalizes publication and does
    // not necessarily start the upload.
