...
bool
ParseDirectBool(const index::BuildParams& params,
                std::string_view key,
                bool fallback) {
    if (!params.contains(key)) {
        return fallback;
    ...
    if (encoded.is_number_integer()) {
        ...
        AssertInfo(value == 0 || value == 1,
                   "direct-build parameter {} must be boolean",
                   key);
        return value != 0;
    }
    AssertInfo(
        encoded.is_string(), "direct-build parameter {} must be boolean", key);
    auto text = encoded.get<std::string>();
    std::transform(text.begin(), text.end(), text.begin(), [](char c) {
        return static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
    });
    AssertInfo(text == "true" || text == "false" || text == "0" || text == "1",
               "direct-build parameter {} must be boolean",
               key);
...
DirectNested(const index::BuildParams& params) {
    ...
    for (const auto key : {std::string_view("nested"),
        ...
        const auto value = ParseDirectBool(params, key, false);
        AssertInfo(!nested.has_value() || *nested == value,
                   "direct-build nested aliases conflict");
        nested = value;
    }
...
IndexNonEncoding(const index::BuildParams& params) {
    if (!params.contains(index::INDEX_NON_ENCODING)) {
        return false;
    }
    const auto& value = params.at(index::INDEX_NON_ENCODING);
    if (value.is_boolean()) {
...
AdaptBuildIndexInfo(const proto::indexcgo::BuildIndexInfo& info,
    ...
    AssertInfo((purpose == BuildPurpose::VectorIndex) == vector_field,
               ...
               field_type);
    if (purpose == BuildPurpose::TextIndex) {
        AssertInfo(IsStringType(field_type),
    ...
    BuildRequest request{
        ...
        .missing_rows = info.lack_binlog_rows(),
...
AdaptDirectBuild(DataType source_type,
    ...
    adapted.params["nullable"] =
        ParseDirectBool(adapted.params, "nullable", false);
