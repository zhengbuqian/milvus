...
class TypedMaterializerBase : public BuildInputMaterializer {
 ...
 private:
    ...
    std::unique_ptr<index::ArtifactBuilder<index::ScalarBuildInput<T>>>
        builder_;
...
class ArrayViewMaterializer final : public TypedMaterializerBase<ArrayView> {
 public:
    ...
    Add(const FieldDataPtr& input) override {
        ...
        batch->owner = input;
        batch->validity = BatchValidity(input);
        const auto* arrays = static_cast<const Array*>(input->Data());
        AssertInfo(arrays != nullptr || input->Length() == 0,
                   "ARRAY batch has null row data");
        batch->views.reserve(input->Length());
        for (size_t row = 0; row < input->Length(); ++row) {
            const bool valid = !batch->validity || batch->validity[row];
            if (valid)
                ValidateArrayRow(arrays[row], element_type_, row, true);
            const bool empty = !valid || arrays[row].length() == 0;
            batch->views.emplace_back(
                empty ? &batch->empty_data
                      : const_cast<char*>(arrays[row].data()),
                empty ? 0 : arrays[row].length(),
                empty ? 0 : arrays[row].byte_size(),
                element_type_,
...
class NestedArrayMaterializer final : public TypedMaterializerBase<T> {
 public:
    ...
    Add(const FieldDataPtr& input) override {
        ...
        const auto* arrays = static_cast<const Array*>(input->Data());
        AssertInfo(arrays != nullptr || input->Length() == 0,
                   "nested ARRAY batch has null row data");
        ...
        for (size_t row = 0; row < input->Length(); ++row) {
            ...
            for (int i = 0; i < arrays[row].length(); ++i) {
                if constexpr (std::is_same_v<T, int8_t> ||
                    ...
                    values->push_back(arrays[row].get_data<T>(i));
                }
            }
        }
        this->AddRows(input->Length());
...
class NestedBoolMaterializer final : public TypedMaterializerBase<bool> {
 public:
    ...
    Add(const FieldDataPtr& input) override {
        ...
        const auto* arrays = static_cast<const Array*>(input->Data());
        AssertInfo(arrays != nullptr || input->Length() == 0,
                   "nested BOOL ARRAY batch has null row data");
        size_t size = 0;
        for (size_t row = 0; row < input->Length(); ++row) {
            if (validity && !validity[row])
                continue;
            ValidateArrayRow(arrays[row], DataType::BOOL, row, false);
            AssertInfo(static_cast<size_t>(arrays[row].length()) <=
                           std::numeric_limits<size_t>::max() - size,
                       "nested BOOL ARRAY element count overflows size_t");
            size += arrays[row].length();
        }
        auto batch = std::make_unique<Batch>();
        ...
        for (size_t row = 0; row < input->Length(); ++row) {
            if (validity && !validity[row])
                continue;
            for (int i = 0; i < arrays[row].length(); ++i) {
                batch->values[next++] = arrays[row].get_data<bool>(i);
            }
        }
        this->AddRows(input->Length());
        batches_.push_back(std::move(batch));
...
class NestedStringMaterializer final
 ...
 public:
    ...
    Add(const FieldDataPtr& input) override {
        ...
        const auto* arrays = static_cast<const Array*>(input->Data());
        AssertInfo(arrays != nullptr || input->Length() == 0,
                   "nested string ARRAY batch has null row data");
        auto batch = std::make_unique<Batch>();
        batch->owner = input;
        for (size_t row = 0; row < input->Length(); ++row) {
            if (validity && !validity[row])
                continue;
            ValidateArrayRow(arrays[row], element_type_, row, false);
            for (int i = 0; i < arrays[row].length(); ++i) {
                batch->values.push_back(
                    arrays[row].get_data<std::string_view>(i));
            }
        }
        this->AddRows(input->Length());
        batches_.push_back(std::move(batch));
