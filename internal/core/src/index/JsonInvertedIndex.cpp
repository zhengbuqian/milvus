...
template <typename T>
void
JsonInvertedIndex<T>::WriteExtraEntries(storage::IndexWriter* writer) {
    std::shared_lock<folly::SharedMutex> lock(this->mutex_);
    if (!this->non_exist_offsets_.empty()) {
...
template <typename T>
void
JsonInvertedIndex<T>::LoadExtraEntries(storage::EntryReader& reader,
                                       const nlohmann::json& meta) {
    bool has_non_exist = meta.value("has_non_exist", false);
    if (has_non_exist) {
    ...
    LOG_INFO("LoadExtraEntries JsonInvertedIndex done, has_non_exist: {}",
             has_non_exist);
}
