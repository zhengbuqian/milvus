...
NgramTokenizerScratchUpperBound(size_t value_len, uintptr_t max_gram) {
    ...
    const auto frontier_count =
        std::min(bytes + 1, max_width + 1);
    const auto frontier_capacity =
        std::max(static_cast<unsigned __int128>(4), frontier_count * 2);
    const auto frontier_bytes =
        frontier_capacity * sizeof(uintptr_t);
    const auto max_token_bytes =
        std::min(bytes, max_width * 4);
    const auto token_capacity =
        max_token_bytes == 0
            ? 0
            : std::max(static_cast<unsigned __int128>(8),
                       max_token_bytes * 2);
...
AddNgramValueToSummary(NgramPreflightSummary& summary,
    ...
    const auto term_bytes =
        (bytes + 1) * width_sum - width_square_sum;
...
NgramInvertedIndex::BuildWithFieldData(const std::vector<FieldDataPtr>& datas) {
    ...
    for (size_t attempt = 0; attempt < 2; ++attempt) {
        ...
        auto status = schema_.data_type() == proto::schema::DataType::JSON
                          ? BuildJsonAttempt(datas, stats)
                          : BuildStringAttempt(datas, stats);
        if (status == BuildAttemptStatus::Applied) {
            status = FinishBuildAttempt();
        }
        if (status == BuildAttemptStatus::Applied) {
            PublishBuildAttempt(std::move(stats));
            return;
        }
...
NgramInvertedIndex::EstimateDirectBuildMemory(
    ...
    if (schema_.data_type() == proto::schema::DataType::JSON) {
        ProcessJsonFieldData<std::string>(
            ...
            JSON_CAST_TYPE,
            JSON_CAST_FUNCTION,
            [this, &summary](const std::string* data,
                             int64_t size,
                             int64_t offset) {
        ...
        for (const auto& data : datas) {
            ...
            for (int64_t row = 0; row < row_count; ++row) {
                if (!schema_.nullable() || data->is_valid(row)) {
                    AddNgramValueToSummary(summary,
                                           values[row].size(),
                                           min_gram_,
                                           max_gram_);
                }
...
NgramInvertedIndex::StartBuildAttempt(NgramBuildBackend backend) {
    ...
    const bool direct = backend == NgramBuildBackend::Direct;
    ...
    LOG_INFO("NGRAM build attempt backend={}, soft_limit_bytes={}, "
             "preflight_required_bytes={}",
             direct ? "direct" : "regular",
             direct_soft_limit_bytes_,
             build_stats_.preflight_required_bytes);
}

NgramInvertedIndex::BuildAttemptStatus
NgramInvertedIndex::BuildStringAttempt(
    const std::vector<FieldDataPtr>& datas, BuildAttemptStats& stats) {
    ...
    auto flush = [this, &rows]() {
        ...
        if (build_stats_.final_backend == NgramBuildBackend::Direct &&
    ...
    for (const auto& data : datas) {
        ...
        for (int64_t row = 0; row < row_count; ++row, ++doc_id) {
            const bool has_value =
                !schema_.nullable() || data->is_valid(row);
            if (!has_value) {
                ...
                stats.total_bytes = SaturatingU128ToU64(
                    ...
                    value.size());
                stats.value_rows = stats.value_rows ==
                                           std::numeric_limits<uint64_t>::max()
                                       ? stats.value_rows
                                       : stats.value_rows + 1;
            }
...
NgramInvertedIndex::BuildJsonAttempt(
    const std::vector<FieldDataPtr>& field_datas, BuildAttemptStats& stats) {
    size_t total_nulls = 0;
    ...
    auto flush = [this,
        ...
        replay_required = TantivyIndexWrapper::RequiresNgramReplay(status);
        pending_rows.clear();
        flat_values.clear();
    };
    ...
    ProcessJsonFieldData<std::string>(
        ...
        JSON_CAST_TYPE,
        JSON_CAST_FUNCTION,
        [&pending_rows, &flat_values, &flush, &stats, &replay_required](
        ...
        [&stats, &replay_required](int64_t offset) {
            if (!replay_required) {
        ...
        [](const Json&, const std::string&, simdjson::error_code) {});
    flush();
    return replay_required ? BuildAttemptStatus::ReplayRequired
                           : BuildAttemptStatus::Applied;
}
...
void
NgramInvertedIndex::PublishBuildAttempt(BuildAttemptStats&& stats) {
    null_offset_ = std::move(stats.null_offsets);
    avg_row_size_ = stats.value_rows > 0
                        ? static_cast<size_t>(stats.total_bytes /
                                              stats.value_rows)
                        : 0;
    LOG_INFO("Ngram index avg_row_size: {} bytes, backend={}, replay_count={}",
             avg_row_size_,
             build_stats_.final_backend == NgramBuildBackend::Direct
                 ? "direct"
                 : "regular",
             build_stats_.replay_count);
}
