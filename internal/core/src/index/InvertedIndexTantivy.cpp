...
template <typename T>
void
InvertedIndexTantivy<T>::WriteEntries(storage::IndexWriter* writer) {
    finish();
    ...
    WriteExtraEntries(writer);
}

template <typename T>
void
InvertedIndexTantivy<T>::LoadEntries(storage::EntryReader& reader,
                                     const Config& config) {
    auto meta_entry = reader.ReadEntry("TANTIVY_META");
    auto mj =
