...
class InvertedIndexTantivy : public ScalarIndex<T> {
 public:
    ...
    void
    WriteEntries(storage::IndexWriter* writer) override;

    void
    LoadEntries(storage::EntryReader& reader, const Config& config) override;
 ...
 protected:
    ...
    // Hook for subclasses to write additional entries after the parent's
    // WriteEntries logic completes.
    virtual void
    WriteExtraEntries(storage::IndexWriter* writer) {
    }

    // Hook for subclasses to load additional entries after the parent's
    // LoadEntries logic completes. The meta parameter is the parsed
    // TANTIVY_META JSON.
    virtual void
    LoadExtraEntries(storage::EntryReader& reader, const nlohmann::json& meta) {
    }
