// Licensed to the LF AI & Data foundation under one
// or more contributor license agreements. See the NOTICE file
// distributed with this work for additional information
// regarding copyright ownership. The ASF licenses this file
// to you under the Apache License, Version 2.0 (the
// "License"); you may not use this file except in compliance
// with the License. You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#pragma once

#include <cstddef>
#include <cstdint>
#include <optional>
#include <string>
#include <string_view>
#include <vector>

#include "common/Types.h"
#include "index/ParamUtils.h"
#include "index/contracts/build/ArtifactBuilder.h"
#include "index/contracts/build/ScalarBuildInput.h"
#include "storage/artifact/Artifact.h"

// FM builder. Accumulates the document text required for suffix-array
// construction; batched input still requires fully resident build state.

namespace milvus::index {

namespace fmindex_params {

inline std::optional<DataType>
ReadDataType(const Config& params, std::string_view key) {
    if (!params.contains(key) || params.at(key).is_null()) {
        return std::nullopt;
    }
    return ParseDataTypeValue(params.at(key), key);
}

}  // namespace fmindex_params

// Build-only parameters. The query-time cost_ratio belongs to the reader and
// is injected separately, not mixed into the builder's configuration.
struct FmIndexBuildParams {
    uint32_t sa_sample_rate{8};
    uint32_t block_bytes{64};
    DataType value_type{DataType::VARCHAR};
    bool nullable{false};
    // Borrowed staging parent used only while Artifact::Serialize runs. The
    // family creates and removes one unique child file, never this directory.
    std::string local_dir;
};

class FmIndexBuilder final
    : public ArtifactBuilder<ScalarBuildInput<std::string_view>> {
 public:
    explicit FmIndexBuilder(FmIndexBuildParams params);

    ~FmIndexBuilder() override;

    storage::ArtifactPtr
        Build(const ScalarBuildInput<std::string_view>& input) &&
        override;

 private:
    void
    AddBatch(const ScalarBuildBatch<std::string_view>& batch);

    FmIndexBuildParams params_;

    // The FM engine requires one combined corpus plus row boundaries; the
    // engine itself injects the out-of-alphabet separators.
    std::vector<uint8_t> corpus_;
    std::vector<size_t> document_offsets_{0};
    TargetBitmap null_bitmap_;
    int64_t total_rows_{0};
};

}  // namespace milvus::index
