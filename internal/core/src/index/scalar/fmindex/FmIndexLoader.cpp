...
ParseRuntimeParams(const Config& params) {
    ...
    std::string_view first_nested_key;
...
GetRequiredMeta(storage::FileSource& source, std::string_view key) {
    ...
    if (!value.has_value()) {
        ThrowInfo(DataFormatBroken, "FM-index V3 metadata {} is missing", key);
    }
    try {
        return value->get<T>();
    } catch (const nlohmann::json::exception& error) {
        ThrowInfo(DataFormatBroken,
                  "invalid FM-index V3 metadata {}: {}",
                  key,
                  error.what());
    }
}
...
LoadStorage(storage::FileSource& source, const storage::LoadOptions& opts) {
    ...
    const auto nullable = GetRequiredMeta<bool>(source, kNullableMeta);
...
RehydratedIndex
FmIndexLoader::OpenForRewrite(storage::FileSource& source,
                              const storage::LoadOptions& opts) {
    auto storage = LoadStorage(source, opts);
