...
ParseValueType(const Config& params) {
    ...
    if (type != DataType::STRING && type != DataType::VARCHAR &&
        type != DataType::TEXT) {
        ThrowInfo(DataTypeInvalid,
...
ResolveLoadOptions(const storage::LoadOptions& opts) {
    ...
    result.enable_mmap =
        opts.enable_mmap || ReadBool(opts.params, ENABLE_MMAP, false) ||
...
class StagingGuard {
 public:
    explicit StagingGuard(std::string directory)
...
std::string
CreateStagingDirectory(const std::string& base) {
    std::error_code error;
    std::filesystem::create_directories(base, error);
    if (error) {
        ThrowInfo(FileCreateFailed,
                  "failed to create marisa staging root {}: {}",
                  base,
                  error.message());
    }
    auto pattern = (std::filesystem::path(base) / "marisa_XXXXXX").string();
    std::vector<char> mutable_pattern(pattern.begin(), pattern.end());
    mutable_pattern.push_back('\0');
    if (mkdtemp(mutable_pattern.data()) == nullptr) {
        ThrowInfo(FileCreateFailed,
                  "failed to create marisa staging directory in {}: {}",
                  base,
                  std::strerror(errno));
    }
...
FileSize(const std::string& path) {
    ...
    if (error || size > std::numeric_limits<size_t>::max()) {
        ThrowInfo(FileReadFailed,
                  "failed to determine marisa entry size for {}: {}",
                  path,
                  error.message());
    }
    ...
    Remove(bool report_error) {
        ...
        if (::unlink(path.c_str()) != 0 && errno != ENOENT && report_error) {
            ThrowInfo(FileWriteFailed,
                      "failed to remove marisa staging file {}: {}",
                      path,
                      std::strerror(errno));
        }
    }
...
class FileDescriptorGuard {
 public:
    ...
    CloseChecked(const std::string& path) {
        ...
        if (::close(fd) != 0) {
            ThrowInfo(FileReadFailed,
                      "failed to close marisa staging file {}: {}",
                      path,
                      std::strerror(errno));
        }
    }
...
ReadAll(int fd, void* output, size_t size, const std::string& path) {
    ...
    while (size != 0) {
        ...
        if (read_size < 0) {
            ThrowInfo(FileReadFailed,
                      "failed to read marisa staging file {}: {}",
                      path,
                      std::strerror(errno));
        }
        if (read_size == 0) {
            ThrowInfo(FileReadFailed,
                      "unexpected EOF while reading marisa staging file {}",
                      path);
        }
        cursor += read_size;
...
class MappingGuard {
 public:
    ...
    MappingGuard(char* data, size_t size) : data_(data), size_(size) {
    }
    ...
    MappingGuard&
    operator=(MappingGuard&& other) noexcept {
        if (this != &other) {
            if (data_ != nullptr) {
                munmap(data_, size_);
            }
...
LoadState(storage::FileSource& source, const storage::LoadOptions& opts) {
    ...
    (void)ReadBool(opts.params, "nullable", false);
    ...
    // this guard removes the paths backing a mapped trie.
    StagingGuard staging(CreateStagingDirectory(staging_root));
    ...
    if (effective.enable_mmap) {
        auto paths = source.ReadEntriesToLocalDir(
            {MARISA_TRIE_INDEX, MARISA_STR_IDS}, staging.Directory());
        if (paths.size() != 2) {
            ThrowInfo(DataFormatBroken,
        ...
        staging.Add(trie_path);
        staging.Add(str_ids_path);
    } else {
        trie_path =
            (std::filesystem::path(staging.Directory()) / MARISA_TRIE_INDEX)
                .string();
        source.ReadEntryToLocalFile(MARISA_TRIE_INDEX, trie_path);
        staging.Add(trie_path);
    }

    const auto trie_bytes = FileSize(trie_path);
    if (trie_bytes == 0) {
    ...
    if (!effective.enable_mmap) {
        auto str_ids = ReadEntryVector<int64_t>(
            source, MARISA_STR_IDS, staging.Directory());
    ...
    const auto str_ids_bytes = FileSize(str_ids_path);
    ...
    if (csr_presence.complete) {
        const auto csr_path =
            (std::filesystem::path(staging.Directory()) / "marisa.csr")
                .string();
        source.ReadEntriesToLocalFile({MARISA_CSR_INDEX, MARISA_CSR_OFFSETS},
                                      csr_path);
        staging.Add(csr_path);
        csr_bytes = FileSize(csr_path);
...
MarisaIndexLoader::OpenIndex(storage::FileSource& source,
    ...
    auto reader = MakeReader(storage);
    return FinishJsonProjectedOpen(
        std::move(projection), source, std::move(reader));
}

RehydratedIndex
MarisaIndexLoader::OpenForRewrite(storage::FileSource& source,
                                  const storage::LoadOptions& opts) {
    auto projection = PrepareJsonProjectedOpen(families::kMarisa, source, opts);
    auto storage = LoadState(source, opts);

    RehydratedIndex inner;
    inner.artifact = std::make_unique<MarisaIndexArtifact>(storage);
    inner.reader = MakeReader(storage);
