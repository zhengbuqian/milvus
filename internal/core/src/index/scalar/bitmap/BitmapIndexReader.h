...
class BitmapIndexReader final : public IndexReaderBase,
 ...
 private:
    // Zone-map style short circuit; kept private, it is not a face.
    bool
...
// The string bitmap reader. Same faces plus `PatternMatchReader`.
class BitmapStringIndexReader final
 ...
 public:
    ...
    // The value face returns an OWNING `std::string` — `owned_t<string_view>`
    // (§5.5). The view decision of §5.1 covers inputs only.
