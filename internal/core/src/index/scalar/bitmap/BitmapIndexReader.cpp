...
BitmapIndexReader<T>::Gather(
    ...
    // segcore/Utils.h:151). §5.5 makes it a face so the implementation can
    // cluster the offsets by its own layout — for a bitmap that means walking
    // the postings once instead of once per row.
}
