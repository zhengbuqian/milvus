...
ArtifactBuilder、Loader、Artifact 和 Reader 保持各自职责；公共代码通过函数复用，
不引入共享的具体 builder/reader 基类。

## 构建、持久化与 Reader

sealed ArtifactBuilder 一次接受 `ScalarBuildInput<T>` 并返回完成的 Artifact。调用方持有
...
外壳；字符串专有的 `PatternMatchReader` 由公共的 string-view-only CRTP adapter 提供，
