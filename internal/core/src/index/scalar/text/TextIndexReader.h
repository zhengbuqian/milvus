...
// (`TextMatchIndex.h:31-50`, §2.2 row 2 — the headline problem of this phase):
//
...
//        §7 point 3: "in-place build != growing" — it is the Builder interface,
...
// each interface has exactly one way to come into existence.
// ==========================================================================
//
...
//
// INTERFACES: `TextMatchReader` only. Deliberately NOT `NullReader`: `IS NULL`
// on a VARCHAR field routes to that field's SCALAR index, not to its text index
// (the text index is a separate artifact under the `text_log/` prefix and is
// only reachable through the text-match expressions). If a null predicate ever
// needs to be answered from the text artifact, add the interface here — do not
// add a throwing stub (§10 rule 4).
