...
RTreeIndexBuilder::RTreeIndexBuilder(RTreeBuildParams params)
    ...
    // RTreeIndex.cpp:76-77) does NOT come here. It is the Appender interface,
    // and it is reached today by segcore reaching in with a `dynamic_cast` plus
    // a call to `InitForBuildIndex(true)` (segcore/FieldIndexing.cpp:766-808) —
    // a downcast that exists only because one class serves both interfaces.
}
