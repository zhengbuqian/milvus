...
void
RTreeIndexArtifact::Serialize(
    storage::FileSink& sink,
    storage::ArtifactSerializationMode mode) const {
    if (mode == storage::ArtifactSerializationMode::FullArtifact) {
        Serialize(sink);
        return;
    }
    AssertInfo(mode == storage::ArtifactSerializationMode::LegacyBinarySet,
               "unknown R-Tree artifact serialization mode");
