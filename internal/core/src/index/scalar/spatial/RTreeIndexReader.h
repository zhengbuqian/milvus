...
//
// FACES: `SpatialReader` + `NullReader`, and nothing else.
...
class RTreeIndexReader final : public IndexReaderBase,
 ...
 public:
    ...
    // offsets. §5 fixes ONE output shape for every face, and sparse offsets are
    // 8 bytes/row against the bitmap's 1 bit/row — a 64x blow-up whenever the
    // candidate set is dense, which for an MBR coarse filter is common. The
    // refine loop on the exec side iterates set bits instead of vector entries;
    ...
    // into a bounding box before the call (:448-459), which is why this face
    // needs no distance parameter.
    TargetBitmap
