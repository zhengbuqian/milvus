...
#include "index/scalar/json/JsonProjectedIndexReader.h"
...
InspectCompleteness(storage::FileSource& source) {
    ...
    const auto meta = source.GetMeta(kHasNonExistMeta);
    if (!meta.has_value()) {
        // An orphan sidecar is deliberately not treated as proof. Old V3
        // artifacts without the typed metadata remain LegacyUnknown.
        return result;
    }

    bool has_non_exist = false;
    try {
        if (!meta->is_boolean()) {
            ThrowInfo(DataFormatBroken,
                      "typed JSON V3 has_non_exist metadata must be boolean");
        }
        has_non_exist = meta->get<bool>();
    } catch (const nlohmann::json::exception& error) {
        ThrowInfo(DataFormatBroken,
                  "invalid typed JSON V3 has_non_exist metadata: {}",
                  error.what());
    }

    const auto has_entry = source.HasEntry(INDEX_NON_EXIST_OFFSET_FILE_NAME);
    if (has_entry != has_non_exist) {
        ThrowInfo(DataFormatBroken,
                  "typed JSON V3 has_non_exist metadata disagrees with "
                  "sidecar presence");
    }
    ...
    const auto declared = source.EntrySize(INDEX_NON_EXIST_OFFSET_FILE_NAME);
    if (declared <= 0 ||
        static_cast<uint64_t>(declared) >
            static_cast<uint64_t>(std::numeric_limits<size_t>::max()) ||
        static_cast<uint64_t>(declared) % sizeof(size_t) != 0) {
        ThrowInfo(DataFormatBroken,
                  "invalid typed JSON V3 non-exist sidecar size {}",
                  declared);
    }
    result.declared_non_exist_bytes = static_cast<size_t>(declared);
    return result;
}
    ...
    CloseChecked(const std::string& path) {
        ...
        if (::close(fd) != 0) {
            ThrowInfo(FileReadFailed,
                      "failed to close typed JSON staging file {}: {}",
                      path,
                      std::strerror(errno));
        }
    }
...
FileSize(const std::string& path) {
    ...
    if (error || size > std::numeric_limits<size_t>::max()) {
        ThrowInfo(FileReadFailed,
                  "failed to determine typed JSON sidecar size for {}: {}",
                  path,
                  error.message());
    }
...
ReadAll(int fd, void* output, size_t size, const std::string& path) {
    ...
    while (size != 0) {
        ...
        if (read_size < 0) {
            ThrowInfo(FileReadFailed,
                      "failed to read typed JSON staging file {}: {}",
                      path,
                      std::strerror(errno));
        }
        if (read_size == 0) {
            ThrowInfo(FileReadFailed,
                      "unexpected EOF while reading typed JSON staging file "
                      "{}",
                      path);
        }
        cursor += read_size;
...
ReadNonExistOffsets(const JsonProjectedOpenPlan& plan,
    ...
    const auto bytes = FileSize(local_path);
    ...
    ReadAll(descriptor.Get(), result.data(), bytes, local_path);
    descriptor.CloseChecked(local_path);
    return result;
}
...
RehydratedIndex
FinishJsonProjectedRewrite(JsonProjectedOpenPlan plan,
                           storage::FileSource& source,
                           RehydratedIndex inner) {
    AssertInfo(inner.artifact != nullptr,
               "typed JSON rewrite requires a non-null inner artifact");
    AssertInfo(inner.reader != nullptr,
               "typed JSON rewrite requires a non-null inner reader");
    if (plan.completeness != JsonProjectionCompleteness::Complete) {
        return inner;
    }
    AssertInfo(plan.cast_type.has_value(),
               "complete typed JSON rewrite lacks a cast type");

    const auto row_count = inner.reader->Count();
    std::vector<size_t> offsets;
    if (plan.has_non_exist_entry) {
    ...
    auto outer_reader = std::make_shared<JsonProjectedIndexReader>(
        ...
        offsets);
    auto outer_artifact =
        std::make_unique<JsonProjectedIndexArtifact>(std::move(inner.artifact),
                                                     std::move(plan.json_path),
                                                     *plan.cast_type,
                                                     row_count,
                                                     std::move(offsets));
    return {.artifact = std::move(outer_artifact),
            .reader = std::move(outer_reader)};
}
