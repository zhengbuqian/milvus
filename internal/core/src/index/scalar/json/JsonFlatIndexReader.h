...
// cannot be consumed away from that column). In refactor phase 1 it moves to
...
//   `JsonFlatIndexReader`      — the routing interface. `Resolve(path,
//                                cast_type)` and `Exists(path)`. Defines NO
...
// A predicate interface over one JSON path. Was `JsonFlatIndexQueryExecutor<T>`
// (`JsonFlatIndex.h:33-726`).
//
// ==========================================================================
// WHAT CHANGES, AND WHY IT IS THE SHARPEST CASE IN THE WHOLE PHASE.
//
...
// The routing interface over one JSON field. Was `JsonFlatIndex`
...
class JsonFlatIndexReader final : public IndexReaderBase,
 ...
 public:
    ...
    // Returns the type-erased base class; the consumer does ONE sibling cast to
