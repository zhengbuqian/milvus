...
// Immutable family state shared by a root reader and a rewrite artifact.
// Directory precedes engine so a mapped Tantivy reader is destroyed before
// its owned staging files. RAM states deliberately retain no directory.
class JsonFlatIndexReaderState final {
 public:
    static std::shared_ptr<const JsonFlatIndexReaderState>
    Create(std::shared_ptr<JsonFlatIndexDirectory> directory,
...
class JsonFlatIndexReader final : public IndexReaderBase,
 ...
 public:
    ...
    // different NullReader view: comparable values at that path.
    TargetBitmap
