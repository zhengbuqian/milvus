...
ValueTypeMatchesCast(DataType value_type, JsonCastType cast_type) {
    switch (cast_type.element_type()) {
        ...
        case JsonCastType::DataType::VARCHAR:
            return IsStringType(value_type);
        default:
            return false;
    }
}
...
void
JsonProjectedIndexSpec::ValidateInner(const IndexReaderBase& inner) const {
...
JsonProjectedIndexReader::JsonProjectedIndexReader(
    ...
    size_t own_heap = sizeof(JsonProjectedIndexReader);
    AddSize(own_heap,
...
JsonProjectedIndexReader::~JsonProjectedIndexReader() = default;

ReaderCaps
JsonProjectedIndexReader::Caps() const {
    return caps_;
}

Domain
JsonProjectedIndexReader::CoordDomain() const {
    return Domain::Row;
}

int64_t
JsonProjectedIndexReader::Count() const {
    return spec_.RowCount();
}

DataType
JsonProjectedIndexReader::ValueType() const {
    return value_type_;
}

int64_t
JsonProjectedIndexReader::MemoryUsage() const {
    return heap_bytes_;
}

cachinglayer::ResourceUsage
JsonProjectedIndexReader::CellByteSize() const {
...
TargetBitmap
JsonProjectedIndexReader::Exists(std::string_view path,
                                 JsonValueType type) const {
    AssertInfo(path == spec_.JsonPath(),
               "typed JSON Exists called for an unregistered path");
    AssertInfo(type == JsonValueType::Any,
               "typed JSON projection can answer only Exists(Any)");
    return exists_.clone();
}

std::vector<JsonCastType>
JsonProjectedIndexReader::CastTypesOf(std::string_view path) const {
