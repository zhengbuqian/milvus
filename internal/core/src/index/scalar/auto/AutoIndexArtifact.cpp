...
AutoIndexArtifact::AutoIndexArtifact(storage::ArtifactPtr inner,
                                     ScalarIndexType selector)
    : inner_(std::move(inner)), selector_(selector) {
    AssertInfo(inner_ != nullptr, "AUTO produced a null artifact");
    AssertInfo(!FamilyFromScalarIndexType(selector_).empty(),
               "AUTO artifact has unsupported selector {}",
...
void
AutoIndexArtifact::Serialize(storage::FileSink& sink) const {
