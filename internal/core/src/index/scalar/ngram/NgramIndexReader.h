...
// THE THREE index -> {exec,segcore,query} edges this wave zeroes (§10 rule 1).
//
...
// ==========================================================================
//
// FACES: `NgramReader` + `NullReader`. Not `ScalarPredicateReader` — an ngram
// index answers no point or range predicate; not `PatternMatchReader` — its
// answer is a SUPERSET (`caps.exact = false`), and conflating it with an exact
// LIKE face is what would let a caller skip the refine step.
