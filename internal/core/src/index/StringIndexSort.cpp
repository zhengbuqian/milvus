...
StringIndexSort::Build(size_t n,
    ...
    if (is_built_)
        return;
    if (n == 0) {
        PanicInfo(DataIsEmpty, "StringIndexSort cannot build null values!");
    }
...
StringIndexSort::Serialize(const Config& config) {
    ...
    // Serialize total number of rows
...
StringIndexSort::Load(milvus::tracer::TraceContext ctx, const Config& config) {
    ...
    auto index_datas =
        file_manager_->LoadIndexToMemory(index_files.value());
...
StringIndexSort::LoadWithoutAssemble(const BinarySet& binary_set,
    ...
    if (version != SERIALIZATION_VERSION) {
        PanicInfo(milvus::ErrorCode::Unsupported,
...
StringIndexSortMmapImpl::LoadFromBinary(const BinarySet& binary_set,
    ...
    if (fd == -1) {
        PanicInfo(DataFormatBroken, "Failed to open mmap file");
    }
    ...
    if (mmap_data_ == MAP_FAILED) {
        PanicInfo(DataFormatBroken, "Failed to mmap file");
    }
