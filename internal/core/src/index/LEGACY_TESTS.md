...
这 18 个 `*Test.cpp` 测的是 W1 已经退役的 surface（`IndexBase`、`ScalarIndex<T>` 的 24 个 virtual、
`IndexFactory` 的 God switch、`InApplyFilter`/`InApplyCallback`、`HybridScalarIndex` 运行时转发类）。
它们**编译不过**——被测的头文件已经不存在了。

**保留它们是有意的**：重写测试时，这批文件是"这一族到底要保证什么"的唯一完整记录。设计文档
[§6.2](../../../docs/design-docs/design_docs/core_refactor/01-scalar-index.md#62-loader-与-io-注入) 与
[§9](../../../docs/design-docs/design_docs/core_refactor/01-scalar-index.md#9-消费者对接) 要求每族一张
**往返一致性测试矩阵**（Builder → Artifact → Serialize → Loader → Reader，多实现逐位一致），那份矩阵还没写。
**在对应族的新测试写完之前，不要删这里的任何一个文件。**
...
| `BoolIndexTest.cpp` | `BoolIndex`（薄别名）、`ScalarIndexSort` | `scalar/bitmap/` |
...
| `HybridScalarIndexTest.cpp` | `HybridScalarIndex` 运行时转发 | `scalar/auto/`——**注意语义变了**：选型成为构建期决策，运行时不再有转发对象可测（§6.3） |
| `ScalarIndexTest.cpp` | 跨族：`ScalarIndex<T>` 的公共 surface | 无单一归属；它测的那层合同已按 §5 拆成各窄面 |

## 重写时要注意的三处语义变化

1. **`HybridScalarIndexTest` 无法直接翻译。** 旧测试断言的是"运行时按基数转发到 bitmap 或 inverted"，
   而新设计里 `auto` 族在 `Seal()` 之后就不存在了——测试对象变成"Builder 选型是否正确 + Loader 是否
   打开了选中的那一族"，断言点从运行时移到构建期产物元数据。
2. **`NgramInvertedIndexTest` 里覆盖 Phase2 的用例属于 exec，不属于索引。** 索引侧只需断言候选是超集
   （`caps.exact == false`），精确验证的用例迁到 exec 的 refine 路径（与 geometry 同构，见 §5.4）。
