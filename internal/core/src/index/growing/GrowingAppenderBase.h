...
// !! CONTRACT GAP — THE APPENDER INTERFACE HAS NO TYPE-ERASED BASE CLASS.
//
// See core_refactor/01-scalar-index.md §7, §7.1, §4.2 (the read side's base
// class) and §3 principle 4 ("templates on the hot path, type erasure only on
...
//
// THE DECISION TAKEN HERE: ADD A BASE CLASS, symmetric to `IndexReaderBase`,
// carrying only what is genuinely T-free. Rationale, in order of weight:
//
...
//  3. `ReaderSnapshotErased()` MAKES THE READ PATH SYMMETRIC WITH SEALED. On
//     the sealed side segcore holds `CacheSlot<IndexReaderBase>` and the
//     consumer sibling-casts to the interface it wants, ONCE PER EXPRESSION
//     NODE (§4.3). With an erased snapshot accessor the growing path is
//     identical — get the base class, cast once to the interface — and segcore
//     never has to downcast the APPENDER at all. It also removes a second gap:
//     the typed `ReaderSnapshot()` in the contract returns exactly ONE
//     interface (`ScalarPredicateReader<T>`), so a growing inverted index could
//     never expose `PatternMatchReader` or `NullReader` on the same snapshot,
//     and a growing RTree could not exist at all (see
//     GrowingSpatialIndexInterface.h). One erased base class, N interfaces,
//     same rule as sealed.
//
...
// cannot answer it. With this base class the whole struct is one
...
// downcasting the appender at all because `ReaderSnapshotErased()` already
// hands back the read-side base class. `caps` stays as it is — it is pure data
// by design (§4.1).
//
...
// in `contracts/GrowingIndex.h` next to the three typed appenders, and the
// typed classes should derive from it — the same relationship `IndexReaderBase`
// has with the query interfaces, except that here it SHOULD be a real base
// class (the typed appenders are classes, not mixins, and there is exactly one
// per object).
...
class GrowingAppenderBase {
 public:
    ...
    // The current immutable snapshot as the type-erased read base class, or
    // null when there is none yet (threshold not reached — §7's "an empty
...
//
// All three land in the same place after refactor phase 1: (1) and (2) become
