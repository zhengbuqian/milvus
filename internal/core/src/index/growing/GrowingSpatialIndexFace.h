...
//     `GrowingScalarIndex<T>` (snapshot face hardwired to
//     `ScalarPredicateReader<T>`), `GrowingTextIndex` (snapshot face
//     `TextMatchReader`) and `GrowingVectorIndex` (snapshot face
//     `VectorSearchReader`).
...
//     it is spatial".
//   - RTree's query face is `SpatialReader` + `NullReader` (§5.6), NOT
//     `ScalarPredicateReader<T>` — §5.6 is explicit that RTree must not
//     implement the predicate face, because `T = std::string` would drag WKB
//     point predicates back in. So `GrowingScalarIndex<std::string>` cannot be
//     its appender: its `ReaderSnapshot()` return type is the one face RTree is
//     forbidden to have.
//
...
// return type can carry only one face. The structural fix is for
// `ReaderSnapshot()` to return `IndexReaderBasePtr` (the type-erased root) and
// let the consumer sibling-cast, exactly as `JsonIndexReader::Resolve` already
// does (§5.7) — one snapshot, N faces, same rule as the sealed side. Declared
// family-locally here rather than by editing contracts/; see the report.
...
class GrowingSpatialIndex {
 public:
    ...
    // for the predicate face: every input here is read-only and points at caller
    // memory.
    virtual void
