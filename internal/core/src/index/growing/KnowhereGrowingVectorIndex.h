...
//
// See core_refactor/01-scalar-index.md §7.1 — "THE APPENDER FACE IS NOT
// SCALAR-ONLY: it exists on the vector side today, and it carries the same
// disease as `IndexBase`" — plus §11.3's face table.
//
...
// is: move the cold-start branch to the Builder face first (segcore gathers once
// from the column when it crosses the threshold, feeds `IndexBuilder<T>`, hands
// the artifact to this appender as its starting point), after which `Append`
// degenerates to the single method the contract declares.
//
...
// here. See the report.
//
// !! Line references point at the pre-W1 tree (master e255009e01) for
// index/ paths; `segcore/FieldIndexing.*` references are to the current tree.
...
class KnowhereGrowingVectorIndex final : public GrowingVectorIndex,
 ...
 public:
    ...
    // happened through the Builder face and its artifact was handed to
    ...
    // THIS METHOD IS THE SEAM BETWEEN THE TWO FACES; it exists precisely so that
    // "build-in-place" and "growing" stop living in one method body.
    void
 ...
 private:
    ...
    // COMPOSED (§3 principle 2). Same engine type the sealed readers hold, which
    // is what lets the published snapshot simply be a `VectorMemReader<T>` — the
    // growing and sealed sides share the READER implementation while their
    // WRITE faces stay separate, which is exactly the split §3 principle 1 is
    // after.
