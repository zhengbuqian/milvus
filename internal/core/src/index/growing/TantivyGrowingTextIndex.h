...
//   2. sealed, build from raw data during load                       -> Builder interface
...
//   3. build service (`FileManagerContext`)                          -> Builder interface
//   4. load an already-built index                                   -> Loader interface
//
...
// BETWEEN SEALED AND GROWING, ONLY THE WRITE INTERFACE DIFFERS (§3 principle 1
// — interfaces are cut by CALLER, not by segment state). One commit produces
// one new reader over the reloaded tantivy generation, shared by every
// concurrent query through `shared_ptr` (§4.3's growing asymmetry).
//
...
class TantivyGrowingTextIndex final : public GrowingTextIndex,
 ...
 public:
    ...
    // appender, not an interface concern (§6.1's "the text builder's tokenizer
