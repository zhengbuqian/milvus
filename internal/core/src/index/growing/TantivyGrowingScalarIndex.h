...
//
// See core_refactor/01-scalar-index.md §7 (the three semantics), §7.1 (the four
// interfaces are unified across families, the method sets inside each interface
// are split by family), §12.6 (which families may lag).
//
...
//
// That reuse is the design's own claim made concrete: THE READ IMPLEMENTATION
// IS SHARED BETWEEN SEALED AND GROWING; ONLY THE WRITE INTERFACE DIFFERS (§3
// principle 1 — interfaces are cut by CALLER, not by segment state; §7.1 — "the
// four interfaces are unified across families, the method sets inside each
// interface are split by family"). A commit produces a new
// `InvertedIndexReader<T>` over the reloaded tantivy generation and every
// concurrent query shares it through `shared_ptr` (§4.3). The vector side does
// the same thing with `VectorMemReader<T>`
...
//  2. INTERFACE-COVERAGE GAP (see GrowingAppenderBase.h): the contract's
//     `ReaderSnapshot()` returns `shared_ptr<const ScalarPredicateReader<T>>` —
//     ONE interface — while the reused snapshot really provides four (base
//     class +
...
class TantivyGrowingScalarIndex final : public GrowingScalarIndex<T>,
 ...
 public:
    ...
    // --- GrowingAppenderBase (family-local base, see that header) -----------
 ...
 private:
    ...
    // index from the flushed column through the BUILDER interface (§3 principle
    // 1, §7 point 3). An appender never produces an artifact.
    void
    CommitAndPublish();
