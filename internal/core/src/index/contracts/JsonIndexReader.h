...
// per-path cast index, and `JsonFlatIndex` (one tantivy index covering all paths
// of the field). JSON SHREDDING IS NOT HERE — `JsonKeyStats` (typed sub-columns,
// the shared BSON sub-column, and the BSON locator inverted index) is a PHYSICAL
// COLUMN LAYOUT, not an index, and moves out of the index component entirely
// (§1). With it goes the `NotImplemented` flood it caused. Within W1 it lands in
// `segcore/json_stats/` (drop the inheritance clause + `git mv`); the end state
// puts the sub-columns in columnar-format and the layout directory in segcore.
//
// The only structural difference from a regular scalar index is ONE EXTRA LEVEL
// OF PATH ADDRESSING: on the same field, `a.b` is an int64 predicate face and
// `a.c` a string one. So this family defines no new query semantics — only
// ROUTING.
//
// `IndexBase::GetCastType` / `Exists` come off the shared root and land here.
...
class JsonIndexReader {
 public:
    ...
    // Is there a usable predicate face at this path? If so, return the
    // type-erased root; the consumer does ONE sibling cast, guided by
