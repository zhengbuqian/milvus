...
// The Appender interface: incremental indexes on growing segments.
//
...
// second, parallel mechanism. This interface is that pattern extracted into one
// contract.
//
// THREE KEY SEMANTICS:
//
...
//     they are different interfaces: build-in-place is a BUILDER (one-shot,
...
// Same shape, different interface on the snapshot.
...
//
// The Appender interface is NOT scalar-only. Both families implement append
// today: `VectorFieldIndexing::AppendSegmentIndexDense`/`Sparse`
// (`FieldIndexing.h:281,287`) and `ScalarFieldIndexing::AppendSegmentIndex`
...
// Interfaces are cut by CALLER (who uses it), families by DATA AND ALGORITHM
// (what structure) — the two are orthogonal. `FieldIndexing`'s error was
// reading "the two families share an interface" as "the two families share a
// set of methods".
//
...
// belongs to the Builder interface. The `built_` branch (`:578-626`) uses only
// this batch for `AddWithDataset` — that is the Appender. The `const
// VectorBase*` (the whole column) in the signature exists ONLY for the first
// branch; the second branch uses it solely to get this batch's validity
...
// the Builder interface first (segcore gathers once from the column when it
// crosses the threshold, feeds `IndexBuilder`, and hands the artifact to the
// appender as its starting point); `Append` then degenerates to the SINGLE
// method below, `VectorBase*` leaves the signature, and dense vs sparse differ
// only in `dim`.
