...
// Takes exclusive ownership before checking the optional capability and
// destroys the artifact shell on success, Unsupported, or conversion failure.
// Callers that also need persistence must Serialize before entering. Missing
// capability never triggers an implicit serialize/load or other IO fallback.
inline IndexReaderBasePtr
ConsumeIndexArtifact(storage::ArtifactPtr artifact) {
    if (artifact == nullptr) {
        ThrowInfo(UnexpectedError, "cannot consume a null index artifact");
    }

    auto* convertible = dynamic_cast<ReaderConvertible*>(artifact.get());
