...
// Exact string-pattern predicates. Pure mixin, independent of IndexReaderBase
...
// inherit an empty primary, so they do not expose PatternMatchReader.
...
class PatternMatchReaderAdapter<Derived,
 ...
 public:
    ...
    ShouldUseForOp(PatternOp op, std::string_view pattern) const override {
        ...
        return PatternMatchReader::ShouldUseForOp(op, pattern);
    }
