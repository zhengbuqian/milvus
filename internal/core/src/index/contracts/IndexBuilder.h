...
//
// See core_refactor/01-scalar-index.md §6 (Builder vs Loader), §6.1 (the
// interface), §6.1.1 (five input forms), §6.1.2 (`BuilderInputSpec`), §6.3
// (hybrid becomes a builder strategy), §11.3 (the interface is shared with the
// vector family).
//
...
//
// BUILDER AND APPENDER ARE TWO INTERFACES, NOT ONE. The Builder is one-shot,
...
//
// ONE INTERFACE FOR BOTH FAMILIES. `T` is the VALUE TYPE, not the family:
// variable length values are expressed with view types (`std::string_view`, an
// array view, a sparse-row view) and dense vectors use `float` with `dim` known
// at construction. Hence no Scalar/Vector prefix (§6.1). Per-family builders —
// the text builder's tokenizer configuration, the json builder's path
// configuration, the vector builder's metric/dim — are IMPLEMENTATIONS OF THIS
// INTERFACE with those knobs as constructor arguments, not new interfaces (§6.1
// last line, §11.3 "Builder: same interface for both families").
//
// WHY THERE IS NO `Consume(ScanCursor&)` (A PULL INTERFACE). The two build
// paths have different sources and different modes, and neither wants pull:
...
// that edge disappears entirely once the four interfaces land.
