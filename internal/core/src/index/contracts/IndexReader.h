...
// The type-erased base class of the Reader interface.
//
// See core_refactor/01-scalar-index.md §4.2, and §4 for why the query
// interfaces do NOT derive from it.
//
// THE QUERY INTERFACES ARE PURE MIXINS. They do not inherit `IndexReaderBase`.
// If each interface did `virtual public IndexReaderBase`, then multiple
// inheritance in an implementation class would need VIRTUAL inheritance to keep
// the base subobject unique; the price is an extra indirection on every access
// to a base-class member from the implementation, and the cast from
// `IndexReaderBase` to a reader interface would have to go through a
// virtual-base `dynamic_cast`. As pure mixins instead: an implementation class
// inherits `IndexReaderBase` and its interfaces NON-virtually, the cast from
// `IndexReaderBase` to a reader interface is a single sibling cast, and the
...
//   - `Build` x3 / `BuildWithDataset` / `BuildWithRawDataForUT`: the Builder
//     interface.
