...
class GrowingIndexSnapshotPin {
 ...
 private:
    friend class GrowingIndex;

    struct Snapshot {
        Snapshot(std::unique_ptr<const IndexReaderBase> value, int64_t end)
            : reader(std::move(value)), covered_row_end(end) {
        }

        const std::unique_ptr<const IndexReaderBase> reader;
...
class GrowingIndex {
 ...
 protected:
    ...
    void
    PublishSnapshot(std::unique_ptr<const IndexReaderBase> reader,
        ...
        // it independently, including after the GrowingIndex is destroyed.
    }
