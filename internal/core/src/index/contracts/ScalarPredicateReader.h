...
// For string families T is `std::string_view` (§5.1): every input on this face
// and on `PatternMatchReader` is read-only and points at caller memory, so
// view-ing has no lifetime problem — the tantivy FFI is ptr+len anyway, and so
// are marisa's `predictive_search` and FMIndex's pattern.
//
// THIS APPLIES TO THE INPUT SIDE ONLY. The value face
...
//   - `Build` / `Size` / `GetIndexType`: not query-face members.
