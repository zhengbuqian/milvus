...
| 对象 | 职责 | 所有权与使用方式 |
|---|---|---|
| `IndexReaderBase` + 查询 mixin | 查询已打开的索引 | load entry/artifact 返回 `unique_ptr`；消费者借用接口，并保留对应 pin |
| `ArtifactBuilder<Input>` | 对完整输入执行一次同步构建 | `Build(input) &&` 返回完成的 Artifact，之后不再使用 builder |
...
| `GrowingIndex` + `Appendable<Batch>` | 长期接收增量输入，发布固定读记录 | Segment 唯一持有 owner；查询持有 `GrowingIndexSnapshotPin`；加载边界用 `Flush` 强制发布已构建 owner 的已接受前缀 |
...
| 文件 | 语义 |
|---|---|
| `IndexReader.h` | 类型擦除基类、坐标域、数量和 reader 自描述 |
| `ReaderCaps.h` | 加载期元数据推导的单个索引能力；选择路径前不 pin |
| `ScalarPredicateReader.h` | 点查与范围查询，输入字符串为借用的 `string_view` |
| `NullReader.h` | 独立于点查的 null 查询；空间等标量类型也可提供，不单设 caps 位 |
| `PatternMatchReader.h` | 精确字符串模式匹配及 typed reader 的 string-view-only CRTP adapter；具体 literal 的代价判断在 pin 后进行 |
| `TextMatchReader.h` | 分词全文查询；独立 text artifact 不隐含 `NullReader` |
| `NgramReader.h` | ngram 候选超集；消费者读取原值并精确验证 |
| `SpatialReader.h` | MBR 候选超集；精确几何关系及距离由消费者验证 |
| `ScalarValueReader.h` | 反查；`Lookup` 返回拥有数据，`Gather` 回调可短暂借用视图 |
| `JsonIndexReader.h` | path/cast 路由到普通谓词 reader，而非新的谓词语义 |
| `VectorReaders.h` | 统一向量 reader：搜索、取值、metadata、nullable mapping、refine 与 embedding-list |

标量实现类非虚继承 `IndexReaderBase` 和支持的纯查询 mixin；mixin 不继承该基类。
向量实现只继承已包含该基类的 `VectorReader`。pin 后只做一次基类到所需查询接口的转换，
后续查询无需知道具体 family。
...
- `CompareOp`、`PatternOp`、`SpatialOp`、`JsonValueType` 使用本地枚举。plan 或引擎枚举在边界转换。
  JSON 路由使用 `JsonCastType`，不能擅自扩大成所有 `DataType` 都受支持。

## 构建、加载与依赖边界

`ArtifactBuilder<Input>` 按完整输入类型模板化，只有一次同步 `Build(input) && -> Artifact`。
...
`unique_ptr<IndexReaderBase>`。加载只产生 Reader，不提供加载后
...
Artifact 实现 `ReaderConvertible`，通过消费式 `IntoReader() &&` 转移已构建状态，
`ConsumeIndexArtifact` 负责能力检查。仅当产物模式支持
...
- `IndexReaderBase::CellByteSize()` 在原生计账可用时报告已打开对象实际拥有的资源：堆结构计入
...
- GrowingIndex 的初始化、append、commit 和读记录发布不消费 Artifact；发布记录由
  GrowingIndex 自己管理。

## Growing 发布与输入约束

- Segment 唯一持有 GrowingIndex；每个发布记录唯一持有 const reader。记录固定 Reader::Count、CoveredRowEnd、validity/offset mapping 和依赖生命周期，但 `const` 不自动保证底层引擎不可变或 Add/Search 并发安全。GrowingIndexSnapshotPin 内部保留记录生命周期，不向消费者暴露共享拥有指针。
...
- Appendable<Batch> 是独立的输入 mixin。ScalarBatch<T>、TextBatch、VectorBatch<T> 是调用期间借用的平坦数据视图；不包含查询能力。不为新增 Reader 能力定义新的 Growing 接口。嵌套输入需显式 offsets/validity 视图。
