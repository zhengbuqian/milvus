...
// The value face: reverse lookup. Pure mixin, does NOT derive from
// `IndexReaderBase` (§4).
//
...
class ScalarValueReader {
 public:
    ...
    // is why the value face does not follow §5.1's `string_view` decision (that
    // decision covers the INPUT side only).
    //
