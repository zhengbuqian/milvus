...
struct ReaderCaps {
    // In / NotIn / Range. Null predicates are NOT represented here: every
    // scalar family really implements them, so `NullReader` is an unconditional
    // interface and gets no bit (§5, "cross-family interface").
