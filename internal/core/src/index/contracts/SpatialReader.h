...
// comparisons). Lifecycle, build, persistence and pin are identical. Splitting a
// component out by operator would be a mis-faceting — "one narrow contract per
// family" is this document's method, and one more spatial family is not an
// exception (§1).
//
// RTree's faces are exactly `SpatialReader` + `NullReader`. Everything it was
// forced to implement — In / NotIn / Range x2 / InApplyFilter / InApplyCallback /
// Reverse_Lookup / Query(DatasetPtr) — is removed: all seven are
// `ThrowInfo(NotImplemented)` shells with zero implementations and zero
// production callers (`RTreeIndex.cpp:462,515,525,534,542`, `RTreeIndex.h:187`).
// `IsNull`/`IsNotNull` are NOT removed — they are real (`RTreeIndex.cpp:469,491`,
// derived from `null_offset_`) and `geo_field IS NULL` reaches the index through
// `PhyNullExpr`. They belong to the unconditional `NullReader` face (§5).
...
//     spatial reader.
//   - `DWithin` IS included, and needs NO `distance` parameter on this face:
//     exec turns the distance into a bounding-box geometry BEFORE the index call
...
class SpatialReader {
 public:
    ...
    // the candidate rows.
    //
    // Output is a `TargetBitmap`, not today's `std::vector<int64_t>` of candidate
    // offsets: that vector is an RTree-era spelling, and §5 fixes ONE output
    // shape for every face. The consumer iterates the set bits; the refine logic
    // is unchanged.
    //
