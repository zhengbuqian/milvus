...
struct LoaderEntry {
    ...
    using OpenFn = IndexReaderBasePtr (*)(storage::FileSource&,
...
concept StaticLoaderProvider = requires(
    ...
    { Provider::Open(source, options) } -> std::same_as<IndexReaderBasePtr>;
};
...
class BuilderRegistry {
 public:
    using Factory = std::function<std::unique_ptr<ArtifactBuilder<Input>>(
        const BuildParams&)>;
    ...
    std::unique_ptr<ArtifactBuilder<Input>>
