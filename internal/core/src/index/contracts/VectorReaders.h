...
// The vector query interfaces. Pure mixins, they do NOT derive from
// `IndexReaderBase` (§4) — same rule as every scalar interface.
//
// See core_refactor/01-scalar-index.md §11.3 (vector's four-interface
// RE-HOMING), §12.1(a) (the interface declares its own narrow parameter type),
// §12.1(b) (the iterator lifetime question, deliberately untouched), §12.1(c)
// (knowhere types are ALLOWED here).
//
...
// family's binding to knowhere is a fait accompli, and wrapping it only pays
// off if the engine is swapped or several engines coexist — a requirement that
// does not exist today. §11.2 rule 5 draws the final boundary: KNOWHERE TYPES
// MAY APPEAR FREELY INSIDE THE VECTOR FAMILY AND NOWHERE ELSE — not in the
// shared base class, not in any scalar family (§10 rule 6). Do not wrap these
// types.
//
// QUERY INTERFACES ARE ZERO-SHARED BETWEEN THE FAMILIES, by decision (§11.2
// rule 2). Scalar is `In/Range/bitmap`, vector is `Search(dataset, params)` —
// there is no abstraction to share, so no cross-family query contract is
// designed. (The Builder and Loader interfaces ARE shared: "feed data -> take
// shape -> produce an Artifact" really is isomorphic across the families, and
// the real difference is input FORM, which cuts ACROSS families — §11.3.)

namespace milvus::index {

// THE INTERFACE DECLARES ITS OWN NARROW PARAMETER TYPE — §12.1(a).
//
...
class VectorSearchReader {
 public:
    ...
    // this refactor, and so refactor phase 1 does not touch it. Do NOT
    // pre-emptively change this to a pin-carrying handle — that would pay
    // contract complexity for an unconfirmed defect. If the audit later
    // confirms eviction is possible, the fix is an anchor like
    // `SearchResult::PinBitset` (`common/QueryResult.h:309`), not a reopening
...
// The value interface — the `GetVector` family, vector's counterpart of
...
// The vector Appender interface is `GrowingVectorIndex`, declared beside
