...
// Verify that forcing kDefaultScalarIndexVersion=1 still produces valid V2
// index files (multiple files) and that the data round-trips correctly.
TEST(ScalarIndexV2Compat, ForceV2Upload) {
    // RAII guard to save and restore kDefaultScalarIndexVersion
    const int32_t original_version = milvus::index::kDefaultScalarIndexVersion;
    struct VersionGuard {
        int32_t saved;
        ~VersionGuard() {
            milvus::index::kDefaultScalarIndexVersion = saved;
        }
    } guard{original_version};

    // Force V2 mode
    milvus::index::kDefaultScalarIndexVersion = 1;
    ...
    // Upload (should use V2 path since kDefaultScalarIndexVersion == 1)
