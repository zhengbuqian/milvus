...
// VECTOR-FAMILY-LOCAL QUERY FACES.
//
// See core_refactor/01-scalar-index.md §11.3 (vector re-homing) and §11.2 rule 2
// (query faces are zero-shared between the families, so the vector family owns
// its own face set).
//
// !! THESE FOUR FACES ARE *NOT* IN `index/contracts/VectorFaces.h` !!
//
...
// GetSparseVector). That set does NOT cover everything today's consumers call on
// a `VectorIndex*`, so re-homing the existing surface with no behaviour change
// (§11.3: "the knowhere interaction moves across verbatim, benchmarks unchanged")
// needs the faces below.
//
...
// "report it, do not bend the contract" . Each face records the production call
// sites that force it. THE CONTRACT OWNER HAS TO DECIDE whether these get folded
// into contracts/VectorFaces.h or stay family-local; until then, treat this file
// as the gap list, not as a settled contract.
//
...
// cannot: those are vector-only vocabulary and the root is shared with the
// scalar families (§10 rule 6 forbids knowhere there — `MetricType` and
...
//
// NOTE THE SHAPE PROBLEM this face inherits and does not fix: it hands out a
// reference to state owned by the reader, and §12.1(b) has already ruled that
// the resulting raw pointer in `SearchResult::vector_iterators_` keeps no pin.
// W1 does not touch that (see the long note in contracts/VectorFaces.h).
...
// the family that cannot do it should simply not implement this face. That is
// why it is a separate mixin rather than three more methods on
// `VectorSearchReader`.
...
// defaults to `ThrowInfo(NotImplemented)` — THE EXACT PATTERN §3 principle 3 and
// §10 rule 4 outlaw. Separating it into its own mixin is what removes the throw:
// a family that has no embedding lists does not inherit this face, and
// `ReaderCaps` needs no bit for it (the caps struct is scalar-shaped today;
// see the report).
