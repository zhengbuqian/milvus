...
VectorDiskReader<T>::GetSparseVector(const DatasetPtr& dataset) const {
    ...
    // and flagged at the top of the header as a contract gap: the face bundles a
    // capability this family does not have, which §3 principle 3 forbids
    // expressing by exception. Not silently "fixed" here — that is the contract
    // owner's call (§11.3 says re-home without redesigning).
    return nullptr;
}
