...
| `contracts/query/VectorReaders.h` | 统一向量查询接口，包含搜索、取值、metadata、nullable、refine 与 embedding-list 操作 |
| `VectorIndexReader` | 非模板统一 reader；运行时仅区分 memory/disk 搜索外壳与 DiskANN beamwidth，物理类型只在取值 leaf 分派 |
| `VectorMemBuilder` | `ArtifactBuilder<VectorBuildInput<T>>`，一次接受完整输入，调用方持有 tensor 及 side inputs |
| `VectorDiskBuilder` | `ArtifactBuilder<PreparedVectorBuildFiles<T>>`，由 knowhere 读取已准备的完整文件 |
...
`VectorIndexReader` 直接继承统一的 `VectorReader`，并按值持有 engine/validity。
...
`VectorReader` 同时提供 metric/dim/knowhere 类型及 iterator 参数准备、借用的 nullable offsets、
...
- 内存 Builder 接收完整 tensor、parent validity、可选 embedding offsets 和标量分类组。
  调用方按 `InputSpec().side_inputs` 预检并交付实际数据；普通 direct 输入借用已有连续
  FieldData，生产输入由物化器持有 compact tensor，不在 Builder 内再存一份 raw 缓冲。
...
- `ArtifactBuilder` 以 `Build(input)` 返回完成的 Artifact，Artifact 负责序列化，构建服务编排上传和
...
- `VectorReader` 同时提供 dense/sparse getter，磁盘后端的 sparse retrieval 仍返回 Unsupported。
