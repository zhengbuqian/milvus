...
#include "index/vector/VectorFamilyFaces.h"
#include "index/vector/VectorValidData.h"
#include "knowhere/expected.h"
#include "knowhere/index/index_node.h"
#include "knowhere/sparse_utils.h"

// READER FACE — knowhere in-memory index families (HNSW, IVF*, FLAT, SCANN,
// sparse WAND/inverted, DataView/interim, ...).
//
...
//   `AddWithDataset`         (VectorIndex.h:66-69,  Unsupported at the root;
//                             the real one is the appender's, see
//                             index/growing/KnowhereGrowingVectorIndex.h)
//   `VectorIterators`        (VectorIndex.h:78-87,  NotImplemented at the root)
//   `GetEmbListByIds`        (VectorIndex.h:118-123, NotImplemented at the root)
//   `Serialize`/`Load`x2/`Upload`/`Build`/`BuildWithDataset` — the whole
//                             lifecycle half of `IndexBase` (§4.2's list).
//
// MULTIPLE INHERITANCE, NON-VIRTUALLY: the root plus N pure-mixin faces (§4's
// note). No face derives from `IndexReaderBase`, so root->face is one sibling
// cast at pin time, once per expression node (§4.3), not per batch.
//
// !! Line references point at the pre-W1 tree (master e255009e01); those files
// are deleted by this change. `git show e255009e01:internal/core/src/index/...`
...
class VectorMemReader final : public IndexReaderBase,
 ...
 public:
    ...
    // --- VectorSearchReader (contracts/VectorFaces.h, §11.3) ---------------
    ...
    // §12.1(b): the raw `IteratorPtr` shape is KEPT AS IS, deliberately. See the
    // full ruling in contracts/VectorFaces.h — the pin-lifetime question is made
    // entirely of existing code, is not amplified by this refactor, and paying
    // contract complexity for an unconfirmed defect would be wrong.
    ...
    // --- VectorIndexMetaReader (family-local, see VectorFamilyFaces.h) ------
