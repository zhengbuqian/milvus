...
// `VectorDiskIndex.cpp:459-460`), and its ARTIFACT is a set of large local files
// that must never be fully resident (VectorDiskArtifact.h). The query face is
// nearly the same shape, which is exactly the split §11.3 predicts: same four
// faces, per-family interfaces inside them.
//
// !! CONTRACT GAP — `VectorValueReader` FORCES A THROW HERE.
// The contract bundles `GetVector` and `GetSparseVector` in one face
// (contracts/VectorFaces.h). DiskANN has no sparse support: today
...
// forbids and §10 rule 4 lints for. Re-homing verbatim (§11.3) carries the throw
// across, so this skeleton keeps it AND flags it rather than silently inventing
// a fix. The fix is a contract-layer decision (split the sparse getter into its
// own mixin, or type the face on the value shape); see the report.
//
