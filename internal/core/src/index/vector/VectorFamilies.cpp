...
// `switch` over `DataType` that news `VectorMemIndex<T>` /
// `VectorDiskAnnIndex<T>` — plus the `VecIndexLoadResource` overloads, every
// one of which took a `storage::FileManagerContext&` and returned an
// `IndexBasePtr` (`index/IndexFactory.h`). After registration, adding or
// removing a vector family touches this directory only, and the top level of
// index/ knows nobody.
...
struct VectorFamilyRegistrar {
    VectorFamilyRegistrar() {
        ...
        // §3 principle 4 made explicit: the builder interface is typed
        // (templates on the data path), the loader interface is type-erased
        // (management interface), and the element type is therefore recovered
        // from artifact metadata inside `VectorMemLoader::OpenIndex`.
    }
};
