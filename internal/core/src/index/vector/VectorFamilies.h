...
// Metadata key carrying the vector element type, so
// `VectorMemLoader::OpenIndex` can recover the `T` that the type-erased loader
// interface erased. The scalar side's `families::kValueTypeMetaKey` serves the
// same purpose; see the note in VectorMemLoader.cpp about why the loader is not
// templated.
