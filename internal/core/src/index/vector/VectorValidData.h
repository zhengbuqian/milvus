...
// a member.
//
// THE MUTATORS AND THE ACCESSORS BELONG TO DIFFERENT FACES, and separating them
// is most of the value of moving this out of a base class:
...
//     `VectorNullableReader` (VectorFamilyFaces.h).
...
class VectorValidData {
 public:
    ...
    // Borrowed. `query/` and `exec/` take this by reference and, in the iterator
    // case, keep a RAW POINTER to it with no pin — see §12.1(b) and the note in
    // contracts/VectorFaces.h. W1 deliberately does not change that.
