    ...
    Read(int destination_fd, size_t size) override {
        try {
            ...
            while (copied < size) {
                ...
                size_t written = 0;
                while (written < read) {
                    const auto result = ::write(destination_fd,
                                                buffer.data() + written,
                                                read - written);
    ...
    Write(const void* ptr, size_t size) override {
        try {
            ...
            while (total < size) {
                const auto result =
                    ::write(fd_,
                            static_cast<const uint8_t*>(ptr) + total,
                            size - total);
                if (result < 0 && errno == EINTR) {
                    continue;
                }
...
VectorDiskBuildFileManager::VectorDiskBuildFileManager(
    std::shared_ptr<VectorDiskLocalFiles> local_files)
