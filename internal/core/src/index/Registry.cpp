...
// needs a definition somewhere, and index/ root is the only place that is
// above every family and below nothing.
//
...
// that had to name every concrete class, i.e. index/ root had a compile-time
// edge to every family. Now each family registers itself from its own .cpp and
// root knows nobody.
//
...
BuilderRegistry<T>::Create(const IndexFamily& family,
    ...
    // builder is an IMPLEMENTATION of the one builder face, not a new face":
    // the per-family parameters are constructor arguments, not extra methods.
    return nullptr;
}
...
// same face (§11.3), which is why there is no Scalar/Vector prefix on
// `IndexBuilder<T>`.
